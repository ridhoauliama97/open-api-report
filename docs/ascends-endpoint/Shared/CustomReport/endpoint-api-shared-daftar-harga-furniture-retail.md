# Dokumentasi Endpoint API Ascend Shared Daftar Harga Furniture Retail

Dokumen ini berisi endpoint internal untuk test/render laporan **Daftar Harga Furniture Retail** Ascend yang memakai Blade shared.
...
### Daftar Harga Furniture Retail

`POST /api/internal/ascends/shared/custom-report/check-price-group-a/daftar-harga-furniture-retail/pdf`

**Route name:** `api.internal.ascends.shared.custom-report.check-price-group-a.daftar-harga-furniture-retail.pdf`

**Controller:** `AscendXmlTestController::apiSharedCustomReportDaftarHargaFurnitureRetailPdf`

**Deskripsi:** Laporan daftar harga furniture retail per kategori group (Merona, Mo.re, Modelux, Grande, Hana/Rak Susun) beserta harga konsumen dan retail diskon 5%.
...
- `PriceLevelName` berawalan `"01. Harga Retail"`.
...
| *(HANA/RAK SUSUN)* | 3101-3105, 2204 (tanpa category header) |
...
| Kolom | Data Source |
|---|---|
| Harga Konsumen | `Price` dari level `01. Harga Retail` |
| Retail Diskon 5% | `PriceAfterDisc` dari level `01. Harga Retail` |
| Isi | `PerDus` dari level `01. Harga Retail` |

### Sorting

`gr_urut` asc (1=Merona, 2=Mo.re, 3=Modelux, 999=Grande/Hana), lalu `urut` asc, lalu deskripsi asc.
