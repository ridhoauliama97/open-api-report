...
- **PDF baru/dimigrasi wajib jalur Gotenberg**: `$pdfGenerator->renderHtml($view, $data)` + `$pdfGenerator->paperMetrics($data)` (+ footer opsional `reports.partials.gotenberg-footer`) → `$gotenbergPdfClient->convertHtml(...)`, catch `GotenbergPdfException` → 502. Contoh lengkap: `MutasiBarangJadiController`; detail & gotchas: `AGENT_INSTRUCTIONS.md` §10.1. Alternatif fasad `spatie/laravel-pdf` (`Pdf::html(...)->format(...)->save(...)`) diarahkan ke pipeline yang sama via `GotenbergPipelineDriver` (throttle/retry/502 tetap jalan) — lihat `AGENT_INSTRUCTIONS.md` §10.2
- **ATURAN BRANCH (WAJIB)**: seluruh kerja migrasi Gotenberg hidup di branch `test/gotenberg-pdf` — **JANGAN PERNAH merge branch ini ke `main`** (main tetap mPDF sampai organisasi memutuskan adopsi). Push hanya ke `origin/test/gotenberg-pdf` dan `utama/test/gotenberg-pdf`. P3 (trait `BuildsGotenbergPdfResponses`, retry, throttle) — **DONE** (commit ce7409b/b581209/4399065): trait di `app/Http/Controllers/Concerns/` (method `buildGotenbergPdfResponse`, 212 call site), retry ConnectionException + backoff/jitter di `GotenbergPdfClient`, semaphore 3 bucket via `GotenbergThrottle` + `GotenbergBucketContext` — JANGAN diusulkan ulang. Follow-up terpisah: tuning `GOTENBERG_MAX_*` via docker stats real, dan font-contract Chromium cold/warm (temuan: font embed fluktuatif cold vs warm)
...
- `boost.json` registers `opencode` agent + `laravel-best-practices` skill; graphify plugin auto-loads from `.opencode/plugins/graphify.js` (OpenCode V2 format: default export `{ id, setup }`, hook via `ctx.tool.hook`)
