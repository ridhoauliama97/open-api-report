...
class AscendsKehadiranKruStickReportFeatureTest extends TestCase
{
    ...
    public function test_shared_attendance_full_kehadiran_kru_stick_api_can_render_uploaded_xml_as_pdf_without_jwt(): void
    {
        $xml = $this->attendanceXml();

        $service = Mockery::mock(KehadiranKruStickReportService::class);
        $service
        ...
        $this->app->instance(KehadiranKruStickReportService::class, $service);
        $this->app->instance(PdfGenerator::class, $pdfGenerator);

        $response = $this->post('/api/internal/ascends/shared/hrm/attendance-full/kehadiran-kru-stick/pdf', [
            'company' => 'RU',
    ...
    public function test_shared_attendance_full_kehadiran_kru_stick_api_can_render_raw_xml_body_without_jwt(): void
    {
        ...
        $pdfGenerator
            ->shouldReceive('render')
            ->once()
            ->with('ascends.shared.hrm.attendance_full.kehadiran_kru_stick.pdf', Mockery::type('array'))
            ->andReturn('%PDF-1.4 mocked content');
        ...
        $response = $this
            ->call(
                'POST',
                '/api/internal/ascends/shared/hrm/attendance-full/kehadiran-kru-stick/pdf',
                ['company' => 'GSU'],
                [],
                [],
                [
    ...
    public function test_shared_attendance_full_kehadiran_kru_stick_api_rejects_request_without_xml_payload(): void
    {
        ...
        $this->postJson('/api/internal/ascends/shared/hrm/attendance-full/kehadiran-kru-stick/pdf', [
            'company' => 'RU',
        ])
            ->assertUnprocessable()
    ...
    public function test_parser_builds_kru_stick_pivot_rows(): void
    {
        $reportData = app(KehadiranKruStickReportService::class)
    ...
    public function test_pdf_blade_renders_expected_layout(): void
    {
        ...
        $reportData['title'] = 'Laporan Kehadiran Kru Stick (RU)';
    ...
    private function reportData(string $company = 'RU'): array
    {
        return [
            ...
            'printed_by' => 'Ridho',
            'company' => $company,
            'title' => "Laporan Kehadiran Kru Stick ({$company})",
...
<NewDataSet>
    ...
    <{$recordTag}>
        ...
        <Full_x0020_Name>Bukan Kru Stick</Full_x0020_Name>
        <Job_x0020_Title>Ka. Div. Stick &amp; Sawn Timber</Job_x0020_Title>
