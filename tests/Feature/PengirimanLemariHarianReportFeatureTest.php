...
class PengirimanLemariHarianReportFeatureTest extends TestCase
{
...
XML;
    ...
    public function test_pengiriman_lemari_harian_pdf_generation(): void
    {
        ...
        $xmlFile = UploadedFile::fake()->createWithContent('Custom15.xml', self::SAMPLE_XML);

        $response = $this->withHeaders([
            'Authorization' => 'Bearer '.$token,
        ])->postJson('/api/internal/ascends/shared/custom-report/pengiriman-lemari-harian/pdf', [
            'xml_file' => $xmlFile,
            'DB_CompanyName' => 'GSU',
        ...
        $response->assertHeader(
            'Content-Disposition',
            'attachment; filename="Laporan Pengiriman Lemari (Harian) GSU.pdf"; filename*=UTF-8\'\'Laporan%20Pengiriman%20Lemari%20%28Harian%29%20GSU.pdf'
        );
    }
    ...
    public function test_grp_calculation_and_category_grouping(): void
    {
        ...
        $this->assertContains('4TX8P', $grps);
        $this->assertContains('4TX4P', $grps);
    }
    ...
    public function test_empty_data_throws_exception(): void
    {
        ...
        $response = $this->postJson('/api/internal/ascends/shared/custom-report/pengiriman-lemari-harian/pdf', [
            'xml_file' => $xmlFile,
            'DB_CompanyName' => 'GSU',
        ]);
