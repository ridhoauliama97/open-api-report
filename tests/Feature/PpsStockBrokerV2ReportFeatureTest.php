...
class PpsStockBrokerV2ReportFeatureTest extends TestCase
{
    ...
    {
        $this->get('/reports/pps/broker/stock-broker-v2')
            ->assertOk()
            ->assertSee('Generate Laporan Stock Broker (PPS)');
    }
    ...
    public function test_preview_endpoint_returns_json_data(): void
    {
        ...
        $service = Mockery::mock(StockBrokerV2ReportService::class);
        $service->shouldReceive('fetch')->once()->andReturn([
            ['Jenis' => 'Broker A', 'JmlhSak' => 5, 'Berat' => 12.5, 'NamaWarehouse' => 'BROKER'],
        ]);

        $this->app->instance(StockBrokerV2ReportService::class, $service);

        $this->withHeaders($this->authJsonHeaders($user))
            ->postJson('/api/reports/pps/broker/stock-broker-v2', [])
            ->assertOk()
            ->assertJsonPath('message', 'Preview laporan berhasil diambil.')
            ->assertJsonPath('meta.total_rows', 1)
            ->assertJsonPath('data.0.Jenis', 'Broker A');
    }
    ...
    public function test_pdf_download_endpoint_returns_attachment(): void
    {
        ...
        $response = $this->actingAs($user)
            ->post('/reports/pps/broker/stock-broker-v2/download', [])
            ->assertOk()
            ->assertHeader('Content-Type', 'application/pdf');

        $this->assertPdfDisposition($response, 'attachment', 'laporan-stock-broker');
    }
    ...
    public function test_health_endpoint_returns_structure_status(): void
    {
        ...
        $this->withHeaders($this->authJsonHeaders($user))
            ->postJson('/api/reports/pps/broker/stock-broker-v2/health', [])
            ->assertOk()
