...
class MutasiKayuBulatKGReportFeatureTest extends TestCase
{
    ...
    public function test_mutasi_kayu_bulat_kg_form_page_is_accessible(): void
    {
        $this->get('/reports/mutasi/kayu-bulat-kg')
            ->assertOk()
            ->assertSee('Generate Laporan Mutasi Kayu Bulat - Timbang KG (PDF)');
    }

    public function test_mutasi_kayu_bulat_kg_preview_endpoint_returns_json_data(): void
    {
        $user = User::factory()->make(['id' => 1]);

        $service = Mockery::mock(MutasiKayuBulatKGReportService::class);
        $service
            ->shouldReceive('fetch')
            ->once()
        ...
        $this->withHeaders($this->authJsonHeaders($user))
            ->postJson('/api/reports/mutasi-kayu-bulat-kg', [
                'TglAwal' => '2026-01-01',
                'TglAkhir' => '2026-01-31',
            ])
            ->assertOk()
    ...
    public function test_mutasi_kayu_bulat_kg_pdf_download_endpoint_returns_attachment(): void
    {
        $user = User::factory()->make(['id' => 1]);
    ...
    public function test_mutasi_kayu_bulat_kg_health_endpoint_returns_structure_status(): void
    {
        ...
        $this->withHeaders($this->authJsonHeaders($user))
            ->postJson('/api/reports/mutasi-kayu-bulat-kg/health', [
                'TglAwal' => '2026-01-01',
                'TglAkhir' => '2026-01-31',
            ])
            ->assertOk()
