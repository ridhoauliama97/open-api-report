...
class ProduksiFjPerNomorProduksiReportFeatureTest extends TestCase
{
    ...
    /**
     * Execute test produksi fj per nomor produksi pdf download endpoint returns inline via gotenberg logic.
     */
    public function test_produksi_fj_per_nomor_produksi_pdf_download_endpoint_returns_inline_via_gotenberg(): void
    {
        $user = User::factory()->make(['id' => 1]);

        $service = Mockery::mock(ProduksiFjPerNomorProduksiReportService::class);
        $service
            ->shouldReceive('fetch')
            ->once()
            ->with('PRD-2026-001')
        ...
        $this->app->instance(ProduksiFjPerNomorProduksiReportService::class, $service);
        $this->app->instance(PdfGenerator::class, $pdfGenerator);

        $response = $this->actingAs($user)
            ->post('/reports/proses-produksi/produksi-fj-per-nomor-produksi/download', [
                'no_produksi' => 'PRD-2026-001',
            ])
            ->assertOk()
            ->assertHeader('Content-Type', 'application/pdf');

        $this->assertPdfDisposition($response, 'inline', 'laporan produksi fj per nomor produksi');
