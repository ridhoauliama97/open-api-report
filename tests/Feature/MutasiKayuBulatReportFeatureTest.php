...
class MutasiKayuBulatReportFeatureTest extends TestCase
{
    ...
    public function test_mutasi_kayu_bulat_form_page_is_accessible(): void
    {
        $this->get('/reports/mutasi/kayu-bulat')
            ->assertOk()
            ->assertSee('Generate Laporan Mutasi Kayu Bulat (PDF)');
    }

    public function test_mutasi_kayu_bulat_preview_endpoint_returns_json_data(): void
    {
        $user = User::factory()->make(['id' => 1]);

        $service = Mockery::mock(MutasiKayuBulatReportService::class);
        $service
            ->shouldReceive('fetch')
            ->once()
        ...
        $this->withHeaders($this->authJsonHeaders($user))
            ->postJson('/api/reports/mutasi-kayu-bulat', [
                'TglAwal' => '2026-01-01',
                'TglAkhir' => '2026-01-31',
            ])
            ->assertOk()
    ...
    public function test_mutasi_kayu_bulat_pdf_download_endpoint_returns_attachment(): void
    {
        $user = User::factory()->make(['id' => 1]);
        ...
        $this->actingAs($user)
            ->post('/reports/mutasi/kayu-bulat/download', [
                'TglAwal' => '2026-01-01',
                'TglAkhir' => '2026-01-31',
            ])
            ->assertOk()
            ->assertHeader('Content-Type', 'application/pdf')
            ->assertHeader('Content-Disposition', 'attachment; filename="laporan-mutasi-kayu-bulat-2026-01-01-sd-2026-01-31.pdf"');
    }
    ...
    public function test_mutasi_kayu_bulat_health_endpoint_returns_structure_status(): void
    {
        ...
        $this->withHeaders($this->authJsonHeaders($user))
            ->postJson('/api/reports/mutasi-kayu-bulat/health', [
                'TglAwal' => '2026-01-01',
                'TglAkhir' => '2026-01-31',
            ])
            ->assertOk()
