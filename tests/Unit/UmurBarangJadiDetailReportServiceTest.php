...
class UmurBarangJadiDetailReportServiceTest extends TestCase
{
    public function test_fetch_groups_same_jenis_dimension_rows_and_sums_age_periods(): void
    {
        config()->set('reports.umur_barang_jadi_detail.database_connection', null);
        config()->set('reports.umur_barang_jadi_detail.call_syntax', 'query');
        config()->set('reports.umur_barang_jadi_detail.parameter_count', 0);
        config()->set('reports.umur_barang_jadi_detail.query', implode(' UNION ALL ', [
        ...
        $rows = (new UmurBarangJadiDetailReportService)->fetch([
            'Umur1' => 15,
            'Umur2' => 30,
            'Umur3' => 60,
            'Umur4' => 90,
    ...
    private function selectAgeRow(
        ...
        return sprintf(
            'SELECT %s AS Jenis, %s AS NamaBarangJadi, %d AS Tebal, %d AS Lebar, %d AS Panjang, %s AS Period1, %s AS Period2, %s AS Period3, %s AS Period4, %s AS Period5, %F AS Total',
            $jenis,
            $namaBarangJadi,
            $tebal,
            $lebar,
