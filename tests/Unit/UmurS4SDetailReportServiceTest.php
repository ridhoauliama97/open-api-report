...
class UmurS4SDetailReportServiceTest extends TestCase
{
    public function test_fetch_groups_same_jenis_dimension_rows_and_sums_age_periods(): void
    {
        config()->set('reports.umur_s4s_detail.database_connection', null);
        config()->set('reports.umur_s4s_detail.call_syntax', 'query');
        config()->set('reports.umur_s4s_detail.parameter_count', 0);
        config()->set('reports.umur_s4s_detail.query', implode(' UNION ALL ', [
            $this->selectAgeRow("'JABON'", "'A/A'", 22, 43, 1000, 'NULL', 1.0000, 'NULL', 'NULL', 'NULL'),
            $this->selectAgeRow("'JABON'", "'A/A'", 22, 43, 1000, 'NULL', 0.6308, 'NULL', 'NULL', 'NULL'),
            $this->selectAgeRow("'JABON'", "'A/A'", 22, 43, 900, 0.2500, 'NULL', 'NULL', 'NULL', 'NULL'),
        ]));

        $rows = (new UmurS4SDetailReportService)->fetch([
            'Umur1' => 15,
            'Umur2' => 30,
            'Umur3' => 60,
            'Umur4' => 90,
    ...
    private function selectAgeRow(
        ...
        return sprintf(
            'SELECT %s AS Jenis, %s AS NamaGrade, %d AS Tebal, %d AS Lebar, %d AS Panjang, %s AS Period1, %s AS Period2, %s AS Period3, %s AS Period4, %s AS Period5',
            $jenis,
            $namaGrade,
            $tebal,
            $lebar,
