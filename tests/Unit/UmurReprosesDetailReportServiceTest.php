...
class UmurReprosesDetailReportServiceTest extends TestCase
{
    public function test_fetch_groups_same_jenis_dimension_rows_and_sums_age_periods(): void
    {
        config()->set('reports.umur_reproses_detail.database_connection', null);
        config()->set('reports.umur_reproses_detail.call_syntax', 'query');
        config()->set('reports.umur_reproses_detail.parameter_count', 0);
        config()->set('reports.umur_reproses_detail.query', implode(' UNION ALL ', [
        ...
        $rows = (new UmurReprosesDetailReportService)->fetch([
            'Umur1' => 15,
            'Umur2' => 30,
            'Umur3' => 60,
            'Umur4' => 90,
