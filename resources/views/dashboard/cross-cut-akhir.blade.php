    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-2">Laporan Dashboard Cross Cut Akhir</h1>
                <p class="text-secondary mb-4">
                    Visualisasi arus masuk dan keluar harian per grade berdasarkan
                    <code>SPWps_LapDashboardCCAkhir</code>.
                </p>
                ...
                <form method="GET" action="{{ route('dashboard.cross-cut-akhir.index') }}" class="row g-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('dashboard.cross-cut-akhir.download', ['start_date' => $startDate, 'end_date' => $endDate]) }}"
                            class="btn btn-success">Generate PDF</a>
                        <a href="{{ route('dashboard.cross-cut-akhir.download', ['start_date' => $startDate, 'end_date' => $endDate, 'preview_pdf' => 1]) }}"
                            class="btn btn-outline-primary" target="_blank">Preview PDF</a>
                        <a href="{{ route('dashboard.cross-cut-akhir.preview', ['start_date' => $startDate, 'end_date' => $endDate]) }}"
                    ...
                    <table class="table table-sm table-striped align-middle mb-0">
                        ...
                        <tfoot>
                            <tr class="fw-semibold">
                                <td>S Akhir</td>
                                @foreach ($columns as $column)
                                    <td class="text-end">{{ $fmt1($sAkhirByColumn[$column] ?? 0) }}</td>
