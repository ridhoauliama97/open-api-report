    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-2">Laporan Dashboard S4S v2</h1>
                <p class="text-secondary mb-4">
                    Visualisasi arus masuk dan keluar harian per grade berdasarkan
                    <code>SPWps_LapDashboardS4S2</code>.
                </p>
                ...
                <form method="GET" action="{{ route('dashboard.s4s-v2.index') }}" class="row g-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4 d-flex align-items-end gap-2">
                        ...
                        <a href="{{ route('dashboard.s4s-v2.download', ['start_date' => $startDate, 'end_date' => $endDate]) }}"
                            class="btn btn-success">Generate PDF</a>
                        <a href="{{ route('dashboard.s4s-v2.download', ['start_date' => $startDate, 'end_date' => $endDate, 'preview_pdf' => 1]) }}"
                            class="btn btn-outline-primary" target="_blank">Preview PDF</a>
                        <a href="{{ route('dashboard.s4s-v2.preview', ['start_date' => $startDate, 'end_date' => $endDate]) }}"
                        ...
                        <thead>
                            <tr>
                                ...
                                @foreach ($columns as $column)
                                    <th colspan="2"
                                        class="text-center {{ in_array($column, $highlightColumns, true) ? 'highlight-col' : '' }}">
                                        {{ $column }}</th>
                                @endforeach
                            </tr>
                            <tr>
                            ...
                            @forelse ($rows as $row)
                                <tr>
                                    ...
                                    @foreach ($columns as $column)
                                        ...
                                        @endphp
                                        <td
                                            class="text-end {{ $isHighlight && abs($inflow) >= 0.000001 ? 'highlight-col' : '' }}">
                                            {{ abs($inflow) < 0.000001 ? '' : $fmt1($inflow) }}</td>
                                        <td
                                            class="text-end {{ $isHighlight && abs($outflow) >= 0.000001 ? 'highlight-col' : '' }}">
                        ...
                        <tfoot>
                            <tr class="fw-semibold">
                                ...
                                @foreach ($columns as $column)
                                    ...
                                    @endphp
                                    <td class="text-end">{{ abs($sAkhirValue) < 0.000001 ? '' : $fmt1($sAkhirValue) }}
                                    </td>
                                    <td class="text-end">{{ abs($sAkhirValue) < 0.000001 ? '' : $fmtPct($pctValue) }}
                                    </td>
                                @endforeach
                            </tr>
                            <tr class="fw-semibold">
