...
<head>
    ...
    <title>Generate Laporan Harian Hasil Spanner Produksi</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Harian Hasil Spanner Produksi (PPS)</h1>
                <p class="text-secondary mb-4">Sumber data utama: stored procedure <code>SP_LapHasilProduksiHarianSpanner</code> pada database PPS.</p>
                ...
                <form method="POST" action="{{ route('reports.pps.inject.spanner.spanner-produksi.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-7">
                        <label for="no_produksi" class="form-label">No Produksi</label>
                        <input type="text" id="no_produksi" name="no_produksi" class="form-control" required list="recentNoProduksiList" value="{{ old('no_produksi') }}" placeholder="Contoh: BJ.0000000173">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            previewButton.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch('{{ route('reports.pps.inject.spanner.spanner-produksi.preview') }}', {
                        method: 'POST',
                        headers: {
