...
<head>
    ...
    <title>Generate Laporan Mutasi Reject</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Mutasi Reject (PPS)</h1>
                <p class="text-secondary mb-4">
                    Sumber data: stored procedure <code>SP_PPSLapMutasiReject</code>.
                    Parameter mengikuti SP: <code>@StartDate</code> dan <code>@EndDate</code>.
                </p>
                ...
                <form method="POST" action="{{ route('reports.pps.reject.mutasi-reject.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                        <label for="StartDate" class="form-label">Tanggal Awal</label>
                        <input type="date" id="StartDate" name="StartDate" class="form-control" required
                            value="{{ old('StartDate', old('start_date', old('TglAwal', now()->format('Y-m-d')))) }}">
                    </div>
                    <div class="col-md-6">
                        <label for="EndDate" class="form-label">Tanggal Akhir</label>
                        <input type="date" id="EndDate" name="EndDate" class="form-control" required
                            value="{{ old('EndDate', old('end_date', old('TglAkhir', now()->format('Y-m-d')))) }}">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.pps.reject.mutasi-reject.preview') }}', {
                        method: 'POST',
                        headers: {
