        ...
        <tbody>
            @forelse ($pivotRows as $label => $warehouseData)
                ...
                <tr class="{{ $loop->odd ? 'row-odd' : 'row-even' }}">
                    ...
                    @foreach ($warehouseLabels as $warehouseName)
                        @php
                            $displayMeasure = array_key_first($activeMeasures);
                            $value = $warehouseData[$warehouseName][$displayMeasure] ?? 0.0;
                            $rowTotals[$displayMeasure] += $value;
                        @endphp
            ...
            <tr class="total-row">
                ...
                @foreach ($warehouseLabels as $warehouseName)
                    @php
                        $displayMeasure = array_key_first($activeMeasures);
                        $totalValue = $pivotTotals[$warehouseName][$displayMeasure] ?? 0.0;
                        $grandTotals[$displayMeasure] += $totalValue;
                    @endphp
