...
<head>
    ...
    <title>Generate Laporan Mutasi Mixer PPS</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Mutasi Mixer (PPS)</h1>
                <p class="text-secondary mb-4">
                    Isi tanggal awal dan tanggal akhir, lalu sistem akan mengambil data dari
                    <code>SP_PPSLapMutasiMixer</code> dan <code>SP_PPSLapSubMutasiMixer</code>
                ...
                <form method="POST" action="{{ route('reports.pps.mixer.mutasi-mixer.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.pps.mixer.mutasi-mixer.preview') }}', {
                        method: 'POST',
                        headers: {
