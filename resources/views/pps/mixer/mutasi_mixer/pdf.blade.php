...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        @page { margin: 24mm 12mm 20mm 12mm; footer: html_reportFooter; }
        ...
        .report-table { border-collapse: separate; border-spacing: 0; border: 1px solid #000; }
        thead { display: table-header-group; }
        tfoot { display: table-footer-group; }
        tr { page-break-inside: avoid; page-break-after: auto; }
        th, td { border: 1px solid #000; padding: 2px 4px; vertical-align: middle; }
        th { text-align: center; font-weight: 700; background: #ffffff; color: #000; }
        td.center { text-align: center; }
        td.label { white-space: nowrap; }
        td.number { text-align: right; white-space: nowrap; font-family: "Calibri", "DejaVu Sans", sans-serif; }
        .row-odd td { background: #c9d1df; }
        .row-even td { background: #eef2f8; }
        .totals-row td { font-weight: bold; font-size: 11px; border: 1px solid #000; }
        .footer-left { font-size: 8px; font-style: italic; }
        .footer-right { font-size: 8px; font-style: italic; text-align: right; }
        .headers-row th { font-weight: bold; font-size: 11px; border-top: 0; border-bottom: 1px solid #000; }
        .report-table tbody tr.data-row td.data-cell { border-top: 0 !important; border-bottom: 0 !important; border-left: 1px solid #000 !important; border-right: 1px solid #000 !important; }
        .table-end-line td { border-top: 1px solid #000 !important; border-right: 0 !important; border-bottom: 0 !important; border-left: 0 !important; padding: 0 !important; height: 0 !important; line-height: 0 !important; background: #fff !important; }
    </style>
</head>
...
<body>
    @php
        ...
        $subRowsData = isset($subRows) && is_iterable($subRows) ? (is_array($subRows) ? $subRows : collect($subRows)->values()->all()) : [];
        usort($rowsData, static fn(array $a, array $b): int => strcmp((string) ($a['Jenis'] ?? ''), (string) ($b['Jenis'] ?? '')));
        usort($subRowsData, static fn(array $a, array $b): int => strcmp((string) ($a['Jenis'] ?? ''), (string) ($b['Jenis'] ?? '')));
        ...
        $fmt = static function ($value, bool $blankWhenZero = true): string {
            ...
            $float = (float) $value;
            if ($blankWhenZero && abs($float) < 0.0000001) { return ''; }
            return number_format($float, 2, '.', ',');
        };
        $num = static fn(array $row, string $key): float => is_numeric($row[$key] ?? null) ? (float) $row[$key] : 0.0;
        $totals = ['BeratAwal' => 0.0, 'MixProdOutput' => 0.0, 'MixInjectOutput' => 0.0, 'MixBSUOutput' => 0.0, 'BeratMasuk' => 0.0, 'InjectInput' => 0.0, 'BrokInput' => 0.0, 'MixerInput' => 0.0, 'BSUInput' => 0.0, 'BeratKeluar' => 0.0, 'BeratAkhir' => 0.0];
        $subTotals = ['MixInputBroker' => 0.0, 'MixInputMix' => 0.0, 'MixInputGil' => 0.0, 'MixInputBB' => 0.0];
    @endphp

    <h1 class="report-title">Laporan Mutasi Mixer</h1>
            ...
            <table class="report-table">
                <thead>
                    ...
                    <tr class="headers-row">
                        ...
                        <th style="width: 78px;">Inject<br>Output</th>
                ...
                <tbody>
                    @forelse ($rowsData as $row)
                        @php
                            ...
                            $totals['MixInjectOutput'] += $mixInjectOutput;
                            $totals['MixBSUOutput'] += $mixBsuOutput;
                            $totals['BeratMasuk'] += $beratMasuk;
                            $totals['InjectInput'] += $injectInput;
                            $totals['BrokInput'] += $brokInput;
                            $totals['MixerInput'] += $mixerInput;
                        ...
                        <tr class="data-row {{ $loop->odd ? 'row-odd' : 'row-even' }}">
                            ...
                            <td class="number data-cell">{{ $fmt($mixProdOutput, true) }}</td>
                            <td class="number data-cell">{{ $fmt($mixInjectOutput, true) }}</td>
                            <td class="number data-cell">{{ $fmt($mixBsuOutput, true) }}</td>
                    ...
                    @if (!empty($rowsData))
                        <tr class="totals-row">
                            ...
                            <td class="number">{{ $fmt($totals['MixProdOutput'], true) }}</td>
                            <td class="number">{{ $fmt($totals['MixInjectOutput'], true) }}</td>
                            <td class="number">{{ $fmt($totals['MixBSUOutput'], true) }}</td>
                            ...
                            <td class="number">{{ $fmt($totals['MixerInput'], true) }}</td>
            ...
            <table class="report-table">
                <thead>
                    <tr class="headers-row">
                        ...
                        <th style="width: 105px;">Input Broker</th>
                        <th style="width: 105px;">Input Mixer</th>
                        <th style="width: 105px;">Input Gilingan</th>
                        <th style="width: 105px;">Input BB</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr class="table-end-line">
                ...
                <tbody>
                    ...
                    @empty
                        <tr class="data-row row-even"><td class="data-cell" colspan="5" style="text-align: center;">Tidak ada data sub mutasi.</td></tr>
                    @endforelse
                    <tr class="totals-row">
                        <td style="text-align:center;">Total :</td>
                        <td class="number">{{ $fmt($subTotals['MixInputBroker'], true) }}</td>
                        <td class="number">{{ $fmt($subTotals['MixInputMix'], true) }}</td>
                        <td class="number">{{ $fmt($subTotals['MixInputGil'], true) }}</td>
                        <td class="number">{{ $fmt($subTotals['MixInputBB'], true) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
