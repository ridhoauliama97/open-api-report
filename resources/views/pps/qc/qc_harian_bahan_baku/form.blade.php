...
<head>
    ...
    <title>Generate Laporan QC Harian Bahan Baku</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan QC Harian Bahan Baku (PPS)</h1>
                <p class="text-secondary mb-4">
                    Sumber data: stored procedure <code>SP_LabelHasilQCBB</code>.
                    Input report menggunakan <code>Tanggal</code>, lalu sistem akan mengambil seluruh label bahan baku pada tanggal tersebut
                    dan memproses data QC dari SP.
                </p>
                ...
                <form method="POST" action="{{ route('reports.pps.qc.qc-harian-bahan-baku.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                        <label for="Tanggal" class="form-label">Tanggal</label>
                        <input type="date" id="Tanggal" name="Tanggal" class="form-control" required
                            value="{{ old('Tanggal', old('report_date', now()->format('Y-m-d'))) }}">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!previewButton || !previewWrapper || !previewOutput || !tanggalInput) {
                return;
            }
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.pps.qc.qc-harian-bahan-baku.preview') }}', {
                        method: 'POST',
                        headers: {
