...
<head>
    ...
    <title>Generate Laporan QC Harian Broker</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan QC Harian Broker (PPS)</h1>
                <p class="text-secondary mb-4">
                    Sumber data: stored procedure <code>SP_LaporanHasilQCHarianBroker</code>.
                    Input report menggunakan <code>EndDate</code>, lalu sistem akan mengambil hasil QC harian broker
                ...
                <form method="POST" action="{{ route('reports.pps.qc.qc-harian-broker.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                        <label for="EndDate" class="form-label">Tanggal</label>
                        <input type="date" id="EndDate" name="EndDate" class="form-control" required
                            value="{{ old('EndDate', old('report_date', now()->format('Y-m-d'))) }}">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.pps.qc.qc-harian-broker.preview') }}', {
                        method: 'POST',
                        headers: {
