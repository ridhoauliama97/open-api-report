...
<head>
    ...
    <title>Generate Laporan Mutasi Bahan Baku</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Mutasi Bahan Baku (PPS)</h1>
                <p class="text-secondary mb-4">
                    Sumber data: stored procedure <code>SP_PPSLapMutasiBahanBaku</code> pada database PPS.
                </p>
                ...
                <form method="POST" action="{{ route('reports.pps.bahan-baku.mutasi-bahan-baku.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-6">
                        <label for="TglAwal" class="form-label">Tanggal Awal</label>
                        <input type="date" id="TglAwal" name="TglAwal" class="form-control" required
                            value="{{ old('TglAwal', old('start_date', now()->format('Y-m-d'))) }}">
                    </div>
                    <div class="col-md-6">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.pps.bahan-baku.mutasi-bahan-baku.preview') }}', {
                        method: 'POST',
                        headers: {
