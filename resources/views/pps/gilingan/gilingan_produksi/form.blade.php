...
<head>
    ...
    <title>Generate Laporan Harian Hasil Gilingan Produksi</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Harian Hasil Gilingan Produksi (PPS)</h1>
                <p class="text-secondary mb-4">Sumber data utama: stored procedure <code>SP_LapHasilProduksiHarianGilingan</code> pada database PPS.</p>

                @if ($errors->any())
                    <div class="alert alert-danger"><ul class="mb-0">@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>
                @endif

                <form method="POST" action="{{ route('reports.pps.gilingan.gilingan-produksi.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-7">
                        <label for="no_produksi" class="form-label">No Gilingan Produksi</label>
                        <input type="text" id="no_produksi" name="no_produksi" class="form-control" required list="recentNoProduksiList" value="{{ old('no_produksi') }}" placeholder="Contoh: W.0000004456">
                ...
                @if (!empty($recentNoProduksi))
                    <div class="mt-4">
                        <h2 class="h6 mb-2">No Gilingan Produksi Terbaru</h2>
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered align-middle mb-0 bg-white">
                                <thead class="table-light"><tr><th>No Produksi</th><th>Tanggal</th><th>Shift</th></tr></thead>
                                <tbody>
                                    @foreach ($recentNoProduksi as $item)
                                        <tr><td>{{ $item['no_produksi'] }}</td><td>{{ $item['tanggal'] }}</td><td>{{ $item['shift'] }}</td></tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (!previewButton || !previewWrapper || !previewOutput || !noProduksiInput) return;
            previewButton.addEventListener('click', async function () {
                previewWrapper.classList.remove('d-none');
                previewOutput.textContent = 'Loading...';
                try {
                    const response = await fetch('{{ route('reports.pps.gilingan.gilingan-produksi.preview') }}', {
                        method: 'POST',
