...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rangkuman Jumlah Label Input</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rangkuman Jumlah Label Input (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi tanggal awal dan tanggal akhir, lalu sistem akan mengambil data dari
                    SPWps_LapRangkumanJlhLabelInput dan mengunduh file PDF.
                </p>
                @if ($errors->any())
                ...
                <form method="POST" action="{{ route('reports.rangkuman-label-input.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.rangkuman-label-input.preview') }}', {
                            method: 'POST',
                            headers: {
