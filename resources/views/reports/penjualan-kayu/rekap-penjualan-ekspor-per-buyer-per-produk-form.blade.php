...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rekap Penjualan Ekspor Per-Buyer dan Per-Produk</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rekap Penjualan Ekspor Per-Buyer dan Per-Produk (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data dari <code>SP_LapJualPerBuyerPerProduk</code> dengan parameter
                    <code>@TglAwal</code> dan <code>@TglAkhir</code>.
                </p>
                @if ($errors->any())
                ...
                <form method="POST"
                    action="{{ route('reports.penjualan-kayu.rekap-penjualan-ekspor-per-buyer-per-produk.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-4">
                        <label for="start_date" class="form-label">Tanggal Awal</label>
                        <input type="date" id="start_date" name="start_date" class="form-control"
                            value="{{ old('start_date', old('TglAwal', $startDate ?? '')) }}" required>
                    </div>
                    <div class="col-md-4">
                        <label for="end_date" class="form-label">Tanggal Akhir</label>
                        <input type="date" id="end_date" name="end_date" class="form-control"
                            value="{{ old('end_date', old('TglAkhir', $endDate ?? '')) }}" required>
                    </div>
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Generate & Download PDF</button>
                            <button type="submit" class="btn btn-outline-primary"
                                formaction="{{ route('reports.penjualan-kayu.rekap-penjualan-ekspor-per-buyer-per-produk.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.penjualan-kayu.rekap-penjualan-ekspor-per-buyer-per-produk.preview') }}', {
                            method: 'POST',
                            headers: {
