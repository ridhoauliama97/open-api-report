...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Penerimaan Kayu Bulat Per Supplier Bulanan (Grafik)</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Penerimaan Kayu Bulat Per Supplier Bulanan (Grafik) (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi tanggal awal dan tanggal akhir. Data diambil dari
                    <code>SP_LaPenerimaanKayuBulatBulananPerSupplier</code>.
                </p>
                @if ($errors->any())
                ...
                <form method="POST" action="{{ route('reports.kayu-bulat.penerimaan-bulanan-per-supplier-grafik.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="button" id="previewJsonBtn" class="btn btn-outline-secondary">Preview Data (JSON)</button>
                        </div>
                    </div>
                </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.kayu-bulat.penerimaan-bulanan-per-supplier-grafik.preview') }}', {
                        method: 'POST',
                        headers: {
