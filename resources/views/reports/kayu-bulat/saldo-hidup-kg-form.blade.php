...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Saldo Hidup Kayu Bulat - Timbang KG</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Saldo Hidup Kayu Bulat - Timbang KG (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data dari <code>SP_LapSaldoHidupKayuBulatKG</code> dan
                    <code>SP_LapSaldoHidupKayuBulatKGSub</code> tanpa parameter input.
                </p>
                @if ($errors->any())
                ...
                <form method="POST" action="{{ route('reports.kayu-bulat.saldo-hidup-kg.download') }}" class="row g-3">
                    @csrf
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Generate & Download PDF</button>
                            <button type="submit" class="btn btn-outline-primary"
                                formaction="{{ route('reports.kayu-bulat.saldo-hidup-kg.preview-pdf') }}" formtarget="_blank">Preview PDF</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.kayu-bulat.saldo-hidup-kg.preview') }}', {
                        method: 'POST',
                        headers: {
