...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Time Line Kayu Bulat - Harian (JTG/PLI)</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Time Line Kayu Bulat - Harian (JTG/PLI) (PDF)</h1>
                <p class="text-secondary mb-4">
                    Sistem akan mengambil data dari SP_LapTimelineKBHarian.
                </p>
                @if ($errors->any())
                ...
                @php
                    $requiresDateRange = (int) config('reports.timeline_kayu_bulat_harian.parameter_count', 2) >= 2;
                @endphp
                <form method="POST" action="{{ route('reports.kayu-bulat.timeline-kayu-bulat-harian.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-6">
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary"
                                formaction="{{ route('reports.kayu-bulat.timeline-kayu-bulat-harian.preview-pdf') }}"
                                formtarget="_blank">
                                Preview PDF
                            </button>
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.kayu-bulat.timeline-kayu-bulat-harian.preview') }}', {
                            method: 'POST',
                            headers: {
