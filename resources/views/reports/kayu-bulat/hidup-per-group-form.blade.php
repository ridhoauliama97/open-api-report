...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Hidup KB Per Group</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Hidup KB Per Group (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data langsung dari stored procedure <code>sp_LapHidupKBPerGroup</code>.
                </p>
                @if ($errors->any())
                ...
                <form method="POST" action="{{ route('reports.kayu-bulat.hidup-per-group.download') }}" class="row g-3">
                    @csrf
                    <div class="col-12">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.kayu-bulat.hidup-per-group.preview') }}', {
                        method: 'POST',
                        headers: {
