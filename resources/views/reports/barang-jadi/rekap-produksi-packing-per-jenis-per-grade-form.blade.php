...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rekap Produksi Packing Per-Jenis &amp; Per-Grade (m3)</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rekap Produksi Packing Per-Jenis &amp; Per-Grade (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi periode tanggal, lalu sistem akan mengambil data dari
                    <strong>SP_LapRekapProduksiBarangJadiPerJenisPerGrade</strong>.
                </p>
                ...
                <form method="POST"
                    action="{{ route('reports.barang-jadi.rekap-produksi-packing-per-jenis-per-grade.download') }}"
                    class="row g-3">
                    @csrf
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" id="TglAwal" name="TglAwal" class="form-control" required
                            value="{{ old('TglAwal', old('start_date', $defaultStart)) }}">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="date" id="TglAkhir" name="TglAkhir" class="form-control" required
                            value="{{ old('TglAkhir', old('end_date', $defaultEnd)) }}">
                    </div>
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary" name="preview_pdf" value="1"
                                formaction="{{ route('reports.barang-jadi.rekap-produksi-packing-per-jenis-per-grade.preview-pdf') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const startInput = document.getElementById('TglAwal');
            const endInput = document.getElementById('TglAkhir');
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.barang-jadi.rekap-produksi-packing-per-jenis-per-grade.preview') }}', {
                            method: 'POST',
                            headers: {
                            ...
                            body: JSON.stringify({
                                TglAwal: startInput ? startInput.value : null,
                                TglAkhir: endInput ? endInput.value : null,
                            }),
                        });
