...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rekap Produksi Moulding Per-Jenis & Per-Grade (m3)</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rekap Produksi Moulding Per-Jenis & Per-Grade (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi periode tanggal, lalu sistem akan mengambil data dari
                    <strong>SP_LapRekapProduksiMouldingPerJenisPerGrade</strong>.
                </p>
                ...
                <form method="POST"
                    action="{{ route('reports.moulding.rekap-produksi-moulding-per-jenis-per-grade.download') }}"
                    class="row g-3">
                    @csrf
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary" name="preview_pdf" value="1"
                                formaction="{{ route('reports.moulding.rekap-produksi-moulding-per-jenis-per-grade.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.moulding.rekap-produksi-moulding-per-jenis-per-grade.preview') }}', {
                            method: 'POST',
                            headers: {
