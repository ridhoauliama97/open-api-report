...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Flow Produksi Per-Periode</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Flow Produksi Per-Periode (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data langsung dari stored procedure
                    <code>SPWps_LapFlowProduksiPerPeriode</code> dengan parameter
                ...
                <form method="POST" action="{{ route('reports.management.flow-produksi-per-periode.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-4">
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.management.flow-produksi-per-periode.preview') }}', {
                            method: 'POST',
                            headers: {
