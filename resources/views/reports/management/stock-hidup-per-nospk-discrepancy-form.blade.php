...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Stock Hidup Per NoSPK (Discrepancy)</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Stock Hidup Per NoSPK (Discrepancy) (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data langsung dari stored procedure <code>SP_LapSemuaStockHidupPerSPK</code>
                    dengan parameter <code>TglAkhir</code> dan mode discrepancy desktop app.
                </p>
                ...
                <form method="POST" action="{{ route('reports.management.stock-hidup-per-nospk-discrepancy.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-4">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!previewButton || !previewWrapper || !previewOutput || !tglAkhir) {
                return;
            }
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.management.stock-hidup-per-nospk-discrepancy.preview') }}', {
                            method: 'POST',
                            headers: {
