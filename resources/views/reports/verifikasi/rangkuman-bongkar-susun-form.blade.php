...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rangkuman Bongkar Susun</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rangkuman Bongkar Susun (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data langsung dari stored procedure
                    <code>SPWps_LapRangkumanBongkarSusun</code> dengan parameter
                    <code>TglAwal</code>.
                </p>
                ...
                <form method="POST" action="{{ route('reports.verifikasi.rangkuman-bongkar-susun.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-4">
                        <label for="TglAwal" class="form-label">Tanggal</label>
                        <input type="date" id="TglAwal" name="TglAwal" class="form-control"
                            value="{{ old('TglAwal', $reportDate) }}">
                    </div>
                    <div class="col-12">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!previewButton || !previewWrapper || !previewOutput || !tglAwal) {
                return;
            }
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.verifikasi.rangkuman-bongkar-susun.preview') }}', {
                        method: 'POST',
                        headers: {
