...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Umur S4S Detail</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Umur S4S Detail (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi parameter umur (batas hari) Umur 1 sampai Umur 4, lalu sistem akan mengambil data dari
                    <strong>SP_LapUmurS4S</strong> dan mengunduh file PDF.
                </p>
                ...
                <form method="POST" action="{{ route('reports.s4s.umur-s4s-detail.download') }}" class="row g-3">
                    @csrf
                    @php
                        $defaultUmur1 = 15;
                        $defaultUmur2 = 30;
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary" name="preview_pdf" value="1"
                                formaction="{{ route('reports.s4s.umur-s4s-detail.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.s4s.umur-s4s-detail.preview') }}', {
                            method: 'POST',
                            headers: {
