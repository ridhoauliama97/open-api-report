...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rekap Produksi S4S Per-Jenis & Per-Grade (m3)</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rekap Produksi S4S Per-Jenis & Per-Grade (m3) (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi tanggal awal dan tanggal akhir, lalu sistem akan mengambil data dari
                    <strong>SP_LapRekapProduksiS4SPerJenisPerGrade</strong> dan mengunduh file PDF.
                </p>
                ...
                <form method="POST" action="{{ route('reports.s4s.rekap-produksi-s4s-per-jenis-per-grade.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-4">
                        <label for="start_date" class="form-label">Tanggal Awal</label>
                        <input type="date" id="start_date" name="start_date" class="form-control" required
                            value="{{ old('start_date', old('TglAwal', now()->startOfMonth()->format('Y-m-d'))) }}">
                    </div>
                    <div class="col-md-4">
                        <label for="end_date" class="form-label">Tanggal Akhir</label>
                        <input type="date" id="end_date" name="end_date" class="form-control" required
                            value="{{ old('end_date', old('TglAkhir', now()->endOfMonth()->format('Y-m-d'))) }}">
                    </div>
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary" name="preview_pdf" value="1"
                                formaction="{{ route('reports.s4s.rekap-produksi-s4s-per-jenis-per-grade.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.s4s.rekap-produksi-s4s-per-jenis-per-grade.preview') }}', {
                            method: 'POST',
                            headers: {
