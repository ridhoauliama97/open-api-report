...
<head>
    ...
    <title>Generate Laporan Serah Terima ST (Kamar KD)</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Serah Terima ST (Kamar KD)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data dari stored procedure <code>SP_LapSerahTerimaSTKDKeluar</code>.
                </p>
                ...
                <form method="POST" action="{{ route('reports.sawn-timber.serah-terima-st-kamar-kd.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-5">
                        <label for="no_proc_kd" class="form-label">No.Proses KD</label>
                        <input type="text" id="no_proc_kd" name="no_proc_kd" class="form-control"
                            value="{{ old('no_proc_kd') }}" maxlength="13" placeholder="Contoh: H.000771" required>
                    </div>
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Generate & Download PDF</button>
                            <button type="submit" class="btn btn-outline-primary"
                                formaction="{{ route('reports.sawn-timber.serah-terima-st-kamar-kd.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.sawn-timber.serah-terima-st-kamar-kd.preview') }}', {
                            method: 'POST',
                            headers: {
