...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Saldo ST Hidup Per Produk</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Saldo ST Hidup Per-Jenis Per-Tebal (Per-Group Jenis Kayu) (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data langsung dari stored procedure <code>SPWps_LapSTHidupPerProduk</code>
                    tanpa parameter input dan mengunduh file PDF.
                </p>
                @if ($errors->any())
                ...
                <form method="POST" action="{{ route('reports.sawn-timber.saldo-st-hidup-per-produk.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-12">
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.sawn-timber.saldo-st-hidup-per-produk.preview') }}', {
                            method: 'POST',
                            headers: {
