...
<head>
    ...
    <title>Generate Laporan Total Bagus/Kulit Rambung</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Total Bagus/Kulit Rambung (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini menghitung total <code>JmlhBatang</code> berdasarkan nilai
                    <code>IsBagusKulit</code> untuk grade <code>KAYU LAT</code>.
                </p>
                ...
                <form method="POST" action="{{ route('reports.sawn-timber.total-bagus-kulit-rambung.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-12 col-md-6">
                        <label for="report_date" class="form-label">Tanggal Sawmill</label>
                        <input type="date" id="report_date" name="report_date"
                            value="{{ old('report_date', $reportDate ?? now()->toDateString()) }}" class="form-control"
                            required>
                    </div>
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary" name="preview_pdf" value="1"
                                formaction="{{ route('reports.sawn-timber.total-bagus-kulit-rambung.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.sawn-timber.total-bagus-kulit-rambung.preview') }}', {
                            method: 'POST',
                            headers: {
