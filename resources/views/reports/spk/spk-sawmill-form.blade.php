...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan SPK Sawmill</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan SPK Sawmill (PDF)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data dari <code>SP_LapHasilSPKSawmill_h</code> dan
                    <code>SP_LapHasilSPKSawmill_d</code> dengan parameter <code>@NoSPK</code> dan
                    <code>@IdProduk</code>.
                </p>
                @if ($errors->any())
                ...
                <form method="POST" action="{{ route('reports.spk.spk-sawmill.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" id="id_produk" name="id_produk" class="form-control"
                            value="{{ old('id_produk', old('IdProduk', $idProduk ?? '')) }}" min="1" required>
                    </div>
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Generate & Download PDF</button>
                            <button type="submit" class="btn btn-outline-primary"
                                formaction="{{ route('reports.spk.spk-sawmill.preview-pdf') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!previewButton || !previewWrapper || !previewOutput || !noSpkInput || !idProdukInput) {
                return;
            }
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.spk.spk-sawmill.preview') }}', {
                        method: 'POST',
                        headers: {
