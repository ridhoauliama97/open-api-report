...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Umur Finger Joint Detail</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Umur Finger Joint Detail (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi parameter umur (batas hari) Umur 1 sampai Umur 4, lalu sistem akan mengambil data dari
                    <strong>SP_LapUmurFingerJoint</strong> dan mengunduh file PDF.
                </p>
                ...
                <form method="POST" action="{{ route('reports.finger-joint.umur-finger-joint-detail.download') }}"
                    class="row g-3">
                    @csrf
                    @php
                        $defaultUmur1 = 15;
                    ...
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button type="submit" class="btn btn-outline-primary" name="preview_pdf" value="1"
                                formaction="{{ route('reports.finger-joint.umur-finger-joint-detail.preview-pdf') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                const umur1 = umur1Input ? umur1Input.value : null;
                const umur2 = umur2Input ? umur2Input.value : null;
                const umur3 = umur3Input ? umur3Input.value : null;
                const umur4 = umur4Input ? umur4Input.value : null;
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.finger-joint.umur-finger-joint-detail.preview') }}', {
                            method: 'POST',
                            headers: {
