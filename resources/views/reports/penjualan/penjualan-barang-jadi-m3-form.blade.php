...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Penjualan Barang Jadi (M3)</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Penjualan Barang Jadi (M3)</h1>
                <p class="text-secondary mb-4">
                    Laporan ini mengambil data dari stored procedure <code>SP_LapPenjualanBarangJadi</code>
                ...
                <form method="POST" action="{{ route('reports.penjualan.penjualan-barang-jadi-m3.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-4">
                        <label for="no_jual" class="form-label">No Jual</label>
                        <input type="text" id="no_jual" name="no_jual" class="form-control"
                            value="{{ old('no_jual', old('NoJual', '')) }}" placeholder="J.001146" required>
                    </div>
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Generate & Download PDF</button>
                            <button type="submit" class="btn btn-outline-primary"
                                formaction="{{ route('reports.penjualan.penjualan-barang-jadi-m3.preview-pdf') }}"
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.penjualan.penjualan-barang-jadi-m3.preview') }}', {
                            method: 'POST',
                            headers: {
