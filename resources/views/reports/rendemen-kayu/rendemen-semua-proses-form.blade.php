...
<head>
    ...
    <title>Generate Laporan Rendemen Semua Proses</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rendemen Semua Proses</h1>
                <p class="text-secondary mb-4">Pilih periode laporan untuk generate PDF dari <code>SP_LapRekapRendemenSemuaProses</code>.</p>
                @if ($errors->any())
                ...
                @endif
                <form method="POST" action="{{ route('reports.rendemen-kayu.rendemen-semua-proses.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                        <label for="start_date" class="form-label">Tanggal Awal</label>
                        <input type="date" class="form-control" id="start_date" name="start_date" value="{{ old('start_date', old('TglAwal')) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label for="end_date" class="form-label">Tanggal Akhir</label>
                        <input type="date" class="form-control" id="end_date" name="end_date" value="{{ old('end_date', old('TglAkhir')) }}" required>
                    </div>
                    <div class="col-12">
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Generate & Download PDF</button>
                            <button type="submit" class="btn btn-outline-primary" formaction="{{ route('reports.rendemen-kayu.rendemen-semua-proses.preview-pdf') }}" formtarget="_blank">Preview PDF</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route('reports.rendemen-kayu.rendemen-semua-proses.preview') }}', {
                        method: 'POST',
                        headers: {
