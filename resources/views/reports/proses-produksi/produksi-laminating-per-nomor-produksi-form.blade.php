...
<head>
    ...
    <title>Generate Laporan Produksi Per Nomor Produksi Laminating</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Produksi Per Nomor Produksi (Laminating)</h1>
                <p class="text-secondary mb-4">
                    Masukkan nomor produksi, lalu sistem akan mengambil data dari
                    <strong>SPWps_LapProduksiLaminating</strong>.
                </p>
                ...
                <form method="POST"
                    action="{{ route('reports.proses-produksi.produksi-laminating-per-nomor-produksi.download') }}"
                    class="row g-3">
                    @csrf
                    ...
                    <div class="col-md-7">
                        ...
                        <input type="text" id="no_produksi" name="no_produksi" class="form-control" required
                            value="{{ old('no_produksi', old('NoProduksi')) }}" placeholder="Contoh: LMT.0000012345">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton?.addEventListener('click', function() {
                postJson('{{ route('reports.proses-produksi.produksi-laminating-per-nomor-produksi.preview') }}',
                    previewOutput, previewWrapper);
            });

            healthButton?.addEventListener('click', function() {
                postJson('{{ route('reports.proses-produksi.produksi-laminating-per-nomor-produksi.health') }}',
                    healthOutput, healthWrapper);
            });
        });
    </script>
</body>
