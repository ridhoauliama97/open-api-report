...
<head>
    ...
    <title>Generate Laporan Produksi Per Nomor Produksi S4S</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Produksi Per Nomor Produksi (S4S)</h1>
                <p class="text-secondary mb-4">
                    Masukkan nomor produksi, lalu sistem akan mengambil data dari
                    <strong>SPWps_LapProduksiS4S</strong>.
                </p>
                ...
                <form method="POST"
                    action="{{ route('reports.proses-produksi.produksi-s4s-per-nomor-produksi.download') }}"
                    class="row g-3">
                    @csrf
                    ...
                    <div class="col-md-7">
                        ...
                        <input type="text" id="no_produksi" name="no_produksi" class="form-control" required
                            value="{{ old('no_produksi', old('NoProduksi')) }}" placeholder="Contoh: S4S.0000012345">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            previewButton?.addEventListener('click', function() {
                postJson('{{ route('reports.proses-produksi.produksi-s4s-per-nomor-produksi.preview') }}',
                    previewOutput, previewWrapper);
            });

            healthButton?.addEventListener('click', function() {
                postJson('{{ route('reports.proses-produksi.produksi-s4s-per-nomor-produksi.health') }}',
