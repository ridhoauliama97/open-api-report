...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Bahan Terpakai</title>
...
<body class="bg-light">
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Bahan Terpakai (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi satu tanggal laporan, lalu sistem akan mengambil data dari
                    SPWps_LapBahanTerpakai dan SPWps_LapSubBahanTerpakai untuk mengunduh file PDF.
                </p>
                ...
                <form method="POST" action="{{ route('reports.bahan-terpakai.download') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6">
                        <label for="TglAwal" class="form-label">Tanggal</label>
                        <input type="date" id="TglAwal" name="TglAwal" class="form-control" required
                            value="{{ old('TglAwal', old('start_date', old('date'))) }}">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const reportDateInput = document.getElementById('TglAwal');
            ...
            previewButton.addEventListener('click', async function() {
                const tglAwal = reportDateInput.value;
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.bahan-terpakai.preview') }}', {
                            method: 'POST',
                            headers: {
