...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,100..900;1,100..900&display=swap"
        rel="stylesheet">
    <title>Generate Laporan Rekap Rendemen Rambung Per Supplier</title>
    ...
    <main class="container py-5">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-md-5">
                <h1 class="h3 mb-3">Generate Laporan Rekap Rendemen Rambung Per Supplier (PDF)</h1>
                <p class="text-secondary mb-4">
                    Isi tanggal awal dan tanggal akhir, lalu sistem akan mengambil data Rekap Rendemen Rambung Per
                    Supplier
                ...
                <form method="POST"
                    action="{{ route('reports.kayu-bulat.rekap-rendemen-rambung-per-supplier.download') }}"
                    class="row g-3">
                    @csrf
                    <div class="col-md-6">
            ...
            previewButton.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(
                        '{{ route('reports.kayu-bulat.rekap-rendemen-rambung-per-supplier.preview') }}', {
                            method: 'POST',
                            headers: {
