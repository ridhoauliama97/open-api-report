...
<body>
    @php
        ...
        $isAkhirColumn = static function (string $column): bool {
            $normalized = strtoupper((string) preg_replace('/[^a-zA-Z0-9]/', '', $column));

            return in_array($normalized, ['AKHIR', 'SALDOAKHIR'], true);
        };
            ...
            @forelse ($rowsData as $row)
                <tr class="data-row {{ $loop->odd ? 'row-odd' : 'row-even' }}">
                    ...
                    @foreach ($mainColumns as $column)
                        ...
                        @endphp
                        @if ($isNumeric)
                            <td class="number {{ $isJenisColumn($column) ? 'col-jenis' : 'col-uniform' }} data-cell" @if ($isAkhirColumn($column)) style="font-weight: bold;" @endif>
