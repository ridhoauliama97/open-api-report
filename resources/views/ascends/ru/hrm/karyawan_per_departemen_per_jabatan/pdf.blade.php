    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 12%;">Pendidikan<br>Terakhir</th>
                <th style="width: 12%;">Tanggal<br>Masuk</th>
                <th style="width: 18%;">Kelompok<br>Kerja</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($groupedRows as $group)
                ...
                @foreach ($groupRows as $row)
                    <tr class="{{ $loop->odd ? 'row-odd' : 'row-even' }}">
                        ...
                        <td class="center">{{ (string) ($row['Tipe'] ?? '') }}</td>
                        <td class="center">{{ (string) ($row['Level'] ?? '') }}</td>
                        <td class="center">{{ (string) ($row['Pendidikan Terakhir'] ?? '') }}</td>
                        <td class="center nowrap">{{ (string) ($row['Tanggal Masuk'] ?? '') }}</td>
                        <td>{{ (string) ($row['Kelompok Kerja'] ?? '') }}</td>
                    </tr>
                @endforeach
                <tr class="summary-row">
                    <td colspan="9">
                        <table class="summary-table">
                            ...
                            @endforeach
                            @foreach ($summaryPairs($summaryItems($summary['type'] ?? [])) as $pair)
                                <tr>
                                    <td class="summary-label">{{ $loop->first ? 'Akumulasi Tipe' : '' }}</td>
                                    <td class="summary-separator">{{ $loop->first ? ':' : '' }}</td>
                                    <td class="summary-value">{{ $formatSummaryItem($pair[0]) }}</td>
                                    <td class="summary-value">{{ $formatSummaryItem($pair[1]) }}</td>
                                </tr>
                            ...
                            @foreach ($summaryPairs($summaryItems($summary['education'] ?? [])) as $pair)
                                <tr>
                                    <td class="summary-label">{{ $loop->first ? 'Akumulasi Pend. Terakhir' : '' }}
                                    </td>
                                    <td class="summary-separator">{{ $loop->first ? ':' : '' }}</td>
                                    <td class="summary-value">{{ $formatSummaryItem($pair[0]) }}</td>
                                    <td class="summary-value">{{ $formatSummaryItem($pair[1]) }}</td>
            ...
            <tr class="summary-row">
                <td colspan="9">
                    <table class="summary-table">
                        ...
                        @endforeach
                        @foreach ($summaryPairs($summaryItems($grandSummary['type'] ?? [])) as $pair)
                            <tr>
                                <td class="summary-label">{{ $loop->first ? 'Akumulasi Tipe' : '' }}</td>
                                <td class="summary-separator">{{ $loop->first ? ':' : '' }}</td>
                                <td class="summary-value">{{ $formatSummaryItem($pair[0]) }}</td>
                        ...
                        @foreach ($summaryPairs($summaryItems($grandSummary['education'] ?? [])) as $pair)
                            <tr>
                                <td class="summary-label">{{ $loop->first ? 'Akumulasi Pendidikan Terakhir' : '' }}
                                </td>
                                <td class="summary-separator">{{ $loop->first ? ':' : '' }}</td>
                                <td class="summary-value">{{ $formatSummaryItem($pair[0]) }}</td>
                                <td class="summary-value">{{ $formatSummaryItem($pair[1]) }}</td>
