...
<body>
  ...
  @if (count($groups) > 0)
    <table class="data-table">
      <thead>
        <tr>
          <th class="col-akun">Kode & Nama Perkiraan</th>
          <th class="col-keterangan">Keterangan</th>
          <th class="col-angka">Debit</th>
          <th class="col-angka">Kredit</th>
          <th class="col-angka">Saldo</th>
        </tr>
      </thead>
      <tbody>
        ...
        @foreach ($groups as $group)
          ...
          @foreach ($group['items'] as $item)
            @php
              $globalRow++;
              $itemIndex++;
              $amtDb = (float) ($item['amount_db'] ?? 0);
              $amtCr = (float) ($item['amount_cr'] ?? 0);
              $amt = (float) ($item['amount'] ?? 0);
            @endphp
            <tr class="{{ $globalRow % 2 === 0 ? 'row-even' : 'row-odd' }}">
              <td>
                {{ $itemIndex === 1 ? (string) ($group['account_code'] ?? '') . ' ' . (string) ($group['account_name'] ?? '') : '' }}
              </td>
              <td>{{ (string) ($item['description'] ?? '') }}</td>
              <td class="number nowrap">{{ $amtDb > 0 ? formatAmount($amtDb) : '-' }}</td>
              <td class="number nowrap">{{ $amtCr > 0 ? formatAmount($amtCr) : '-' }}</td>
              <td class="number nowrap">{{ formatAmount($amt) }}</td>
            </tr>
          @endforeach
          <tr class="account-subtotal">
            <td colspan="2" class="center">Sub Total
              {{ (string) ($group['account_name'] ?? '') }} </td>
