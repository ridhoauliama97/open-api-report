...
<body>
    @php
        ...
        $headerTitle = trim((string) ($reportData['title'] ?? ''));
        ...
        @foreach ($categories as $category)
            ...
            @if ($category['grp_groups'] !== null)
                ...
                @foreach ($category['grp_groups'] as $group)
                    ...
                    @if (count($grpItems) > 0)
                        <div class="grp-header">{{ $group['grp'] !== '-' ? '- ' . $group['grp'] : '-' }}</div>
                        <table class="data-table">
                            <thead>
                                <tr>
                                    <th style="width: 24%;">Tahun</th>
                                    @foreach ($months as $mNum => $mLabel)
                                        <th style="width: 6%;">{{ $mLabel }}-{{ substr($yearLabel, -2) }}</th>
                                    @endforeach
    ...
    @else
        <table class="data-table">
            <tbody>
                <tr class="empty-row">
                    <td>Tidak ada data pengiriman per kategori tahunan.</td>
                </tr>
            </tbody>
        </table>
    @endif
