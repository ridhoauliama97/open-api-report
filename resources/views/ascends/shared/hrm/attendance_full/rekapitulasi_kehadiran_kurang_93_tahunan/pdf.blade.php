...
<body>
    @php
        ...
        $nameWidth = count($months) > 8 ? 30 : 38;
        $monthWidth = count($months) > 0 ? max(5, min(7, (int) floor((86 - $nameWidth) / (count($months) + 1)))) : 6;
    @endphp
        ...
        <thead>
            ...
            <tr>
                @foreach ($months as $month)
                    <th style="width: {{ $monthWidth }}%;">{{ str_pad((string) $month, 2, '0', STR_PAD_LEFT) }}</th>
                @endforeach
            </tr>
        </thead>
        <tbody>
            ...
            @empty
                <tr class="empty-row">
                    <td colspan="{{ 2 + count($months) }}">Tidak Ada Data</td>
                </tr>
            @endforelse
        </tbody>
    </table>
