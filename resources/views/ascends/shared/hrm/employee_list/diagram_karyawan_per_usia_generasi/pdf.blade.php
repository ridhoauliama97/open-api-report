            ...
            <tr>
                <td>
                    @foreach ($leftDepts as $i => $dept)
                        @php
                            $color = \App\Services\Ascends\Shared\Hrm\DiagramKaryawanPerUsiaGenerasiReportService::CHART_COLORS[$i % 12] ?? [0, 0, 0];
                        @endphp
                        <div>
                ...
                <td>
                    @foreach ($rightDepts as $i => $dept)
                        @php
                            $color = \App\Services\Ascends\Shared\Hrm\DiagramKaryawanPerUsiaGenerasiReportService::CHART_COLORS[($mid + $i) % 12] ?? [0, 0, 0];
                        @endphp
                        <div>
                            <span class="dept-label-checkbox"
                                style="background: rgb({{ $color[0] }},{{ $color[1] }},{{ $color[2] }});">&nbsp;</span>
