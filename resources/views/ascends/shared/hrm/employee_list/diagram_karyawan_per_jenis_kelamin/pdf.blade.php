...
<body>
    ...
    @if ($pieChart !== '')
        <div class="chart-wrapper">
            <img src="{{ $pieChart }}" alt="Diagram Pie Jenis Kelamin">
        </div>
    @endif
            ...
            <tr>
                <td>
                    @foreach ($leftDepts as $i => $dept)
                        @php
                            $color = \App\Services\Ascends\Shared\Hrm\DiagramKaryawanPerJenisKelaminReportService::CHART_COLORS[$i % 12] ?? [0, 0, 0];
                        @endphp
                        <div>
                ...
                <td>
                    @foreach ($rightDepts as $i => $dept)
                        @php
                            $color = \App\Services\Ascends\Shared\Hrm\DiagramKaryawanPerJenisKelaminReportService::CHART_COLORS[($mid + $i) % 12] ?? [0, 0, 0];
                        @endphp
                        <div>
                            <span class="dept-label-checkbox"
                                style="background: rgb({{ $color[0] }},{{ $color[1] }},{{ $color[2] }});">&nbsp;</span>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 52%;">Jenis Kelamin</th>
