            ...
            @forelse ($groupedRows as $group)
                ...
                @foreach ($groupRows as $row)
                    <tr class="{{ $loop->odd ? 'row-odd' : 'row-even' }}">
                        ...
                        <td class="center">{{ (string) ($row['Agama'] ?? '') }}</td>
                    </tr>
                @endforeach
            ...
            <tr class="summary-row">
                <td colspan="6">
                    <table class="summary-table">
                        ...
                        @endforeach
                        @foreach ($summaryPairs($summaryItems($grandSummary['ethnicity'] ?? [])) as $pair)
                            <tr>
                                <td class="summary-label">{{ $loop->first ? 'Akumulasi Etnis' : '' }}</td>
                                <td class="summary-separator">{{ $loop->first ? ':' : '' }}</td>
                                <td class="summary-value">{{ $formatSummaryItem($pair[0]) }}</td>
                                <td class="summary-value">{{ $formatSummaryItem($pair[1]) }}</td>
                            </tr>
