        ...
        <tbody>
            @forelse ($categories as $catIndex => $category)
                @php
                    ...
                    $pcts = $category['percentages'] ?? [];
                @endphp

                <tr class="category-header">
                    <td colspan="7">
                        {{ $category['assembly_name'] }}
                    </td>
                </tr>
                ...
                <tr class="category-summary">
                    <td colspan="7">
                        Sub Total = {{ number_format($category['sub_total'], 0, '.', ',') }}
                    </td>
                </tr>
                <tr class="category-summary">
                    <td colspan="7">
                ...
                <tr class="category-summary">
                    <td colspan="7">
                        Reproses (Kg) = {{ number_format($totals['total_reproses'], 0, '.', ',') }}
                        ({{ $pcts['reproses_pct'] ?? 0 }}%)
                    </td>
                </tr>
                <tr class="category-summary">
                    <td colspan="7">
                        Bonggolan (Kg) = {{ number_format($totals['total_bonggolan'], 0, '.', ',') }}
                        ({{ $pcts['bonggolan_pct'] ?? 0 }}%)
                    </td>
                </tr>
                <tr class="category-summary">
                    <td colspan="7">
                        Downtime Machine (Kg) = 0 (0%)
                    </td>
                </tr>
            @empty
                <tr class="empty-row">
            ...
            @if (!empty($categories))
                ...
                <tr>
                    <td colspan="7">
                        Akumulasi Grand Total Reproses (Kg) =
                        {{ number_format($grandTotals['total_reproses'], 0, '.', ',') }} (100%)
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
                        Akumulasi Grand Total Bonggolan (Kg) =
                        {{ number_format($grandTotals['total_bonggolan'], 0, '.', ',') }} (100%)
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
                        Akumulasi Grand Total Downtime Machine (Kg) =
                        {{ number_format($grandTotals['total_downtime'], 0, '.', ',') }} (0%)
                    </td>
                </tr>
            @endif
        </tbody>
