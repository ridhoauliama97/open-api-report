...
class ProduksiS4sPerNomorProduksiReportService
{
    /**
    ...
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data produksi S4S tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.produksi_s4s_per_nomor_produksi.expected_columns', []);
    ...
    public function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        return [
            'meta' => [
                ...
                'stored_procedure' => (string) config('reports.produksi_s4s_per_nomor_produksi.stored_procedure', 'SPWps_LapProduksiS4S'),
                ...
                'nama_mesin' => $this->firstNonEmptyString($firstRow, ['NamaMesin', 'Mesin', 'Machine']) ?: 'S4S',
                ...
                'input_label' => $inputLabel ?: 'SAWN TIMBER',
                'output_label' => $outputLabel ?: 'S4S',
            ],
    ...
    {
        $configPath = 'reports.produksi_s4s_per_nomor_produksi';
        $connectionName = config("{$configPath}.database_connection");
        $procedure = (string) config("{$configPath}.stored_procedure", 'SPWps_LapProduksiS4S');
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan produksi S4S per nomor produksi belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan produksi S4S per nomor produksi dikonfigurasi untuk SQL Server. '
                . 'Set PRODUKSI_S4S_PER_NOMOR_PRODUKSI_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('PRODUKSI_S4S_PER_NOMOR_PRODUKSI_REPORT_QUERY belum diisi.');
    ...
    private function detectDirection(array $row): ?string
    {
        ...
        if ($process !== '') {
            if (str_contains($process, 'sawn') || str_contains($process, 'timber') || $process === 'st') {
                return 'input';
            }

            if (str_contains($process, 's4s')) {
                return 'output';
            }
        }
