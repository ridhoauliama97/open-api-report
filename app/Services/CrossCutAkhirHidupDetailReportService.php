...
class CrossCutAkhirHidupDetailReportService
{
    /**
    ...
    public function fetch(): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            $noCcaAkhir = trim((string) ($item['NoCCAkhir'] ?? $item['No_CCAkhir'] ?? $item['No'] ?? ''));
            ...
            return [
                'NoCCAkhir' => $noCcaAkhir !== '' ? $noCcaAkhir : null,
        ...
        usort($normalized, static function (array $a, array $b): int {
            ...
            return strcmp((string) ($a['NoCCAkhir'] ?? ''), (string) ($b['NoCCAkhir'] ?? ''));
        });
    ...
    public function healthCheck(): array
    {
        ...
        $expectedColumns = config('reports.cross_cut_akhir_hidup_detail.expected_columns', []);
    ...
    {
        $connectionName = config('reports.cross_cut_akhir_hidup_detail.database_connection');
        $procedure = (string) config('reports.cross_cut_akhir_hidup_detail.stored_procedure', 'SP_LapCCAkhirHidupDetail');
        $syntax = (string) config('reports.cross_cut_akhir_hidup_detail.call_syntax', 'exec');
        $customQuery = config('reports.cross_cut_akhir_hidup_detail.query');
        $parameterCount = (int) config('reports.cross_cut_akhir_hidup_detail.parameter_count', 0);

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan Cross Cut Akhir (Hidup) detail belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan Cross Cut Akhir (Hidup) detail dikonfigurasi untuk SQL Server. '
                . 'Set CROSS_CUT_AKHIR_HIDUP_DETAIL_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'CROSS_CUT_AKHIR_HIDUP_DETAIL_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan CROSS_CUT_AKHIR_HIDUP_DETAIL_REPORT_CALL_SYNTAX=query.',
                );
