...
class RekapProduksiLaminatingConsolidatedReportService
{
    /**
    ...
    public function fetch(string $startDate, string $endDate): array
    {
        ...
        $normalized = array_map(function ($row) use ($eps): array {
            ...
            $output = $this->toFloat($item['OutputLaminating'] ?? null);
            ...
            $m3PerJam = ($jam !== null && abs($jam) > $eps && $output !== null)
                ? $output / $jam
                : null;

            $personHours = ($jam !== null && abs($jam) > $eps && $org > 0) ? ($jam * $org) : null;
            $m3PerJamOrg = ($personHours !== null && abs($personHours) > $eps && $output !== null)
                ? $output / $personHours
                : null;

            $rend = abs($totalInput) > $eps && $output !== null
    ...
    {
        $connectionName = config('reports.rekap_produksi_laminating_consolidated.database_connection');
        $procedure = (string) config(
            'reports.rekap_produksi_laminating_consolidated.stored_procedure',
            'SP_LapRekapProduksiLaminatingConsolidated'
        );
        $syntax = (string) config('reports.rekap_produksi_laminating_consolidated.call_syntax', 'exec');
        $customQuery = config('reports.rekap_produksi_laminating_consolidated.query');
        $parameterCount = (int) config('reports.rekap_produksi_laminating_consolidated.parameter_count', 2);

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan rekap produksi laminating consolidated belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan rekap produksi laminating consolidated dikonfigurasi untuk SQL Server. '
                . 'Set REKAP_PRODUKSI_LAMINATING_CONSOLIDATED_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'REKAP_PRODUKSI_LAMINATING_CONSOLIDATED_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan REKAP_PRODUKSI_LAMINATING_CONSOLIDATED_REPORT_CALL_SYNTAX=query.',
                );
