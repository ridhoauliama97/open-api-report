...
class RekapProduksiLaminatingPerJenisPerGradeReportService
{
    /**
    ...
    public function fetch(string $startDate, string $endDate): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            return [
                ...
                'InMoulding' => $this->toFloat($item['Moulding'] ?? null),
                'InSanding' => $this->toFloat($item['Sanding'] ?? null),
                'InWIP' => $this->toFloat($item['WIP'] ?? $item['InWIP'] ?? null),
                'InReproses' => $this->toFloat($item['Reproses'] ?? null),
                'Output' => $this->toFloat($item['Output'] ?? $item['OutPut'] ?? $item['OutputLaminating'] ?? null),
            ];
        }, $rows);
    ...
    {
        $connectionName = config('reports.rekap_produksi_laminating_per_jenis_per_grade.database_connection');
        $procedure = (string) config(
            'reports.rekap_produksi_laminating_per_jenis_per_grade.stored_procedure',
            'SP_LapRekapProduksiLaminatingPerJenisPerGrade',
        );
        $syntax = (string) config('reports.rekap_produksi_laminating_per_jenis_per_grade.call_syntax', 'exec');
