...
class ProduksiLaminatingPerNomorProduksiReportService
{
    /**
    ...
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data produksi laminating tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.produksi_laminating_per_nomor_produksi.expected_columns', []);
    ...
    public function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        return [
            'meta' => [
                ...
                'stored_procedure' => (string) config('reports.produksi_laminating_per_nomor_produksi.stored_procedure', 'SPWps_LapProduksiLaminating'),
                ...
                'nama_mesin' => $this->firstNonEmptyString($firstRow, ['NamaMesin', 'Mesin', 'Machine']) ?: 'LAMINATING',
                ...
                'input_label' => $inputLabel ?: 'FJ',
                'output_label' => $outputLabel ?: 'LAMINATING',
            ],
    ...
    {
        $configPath = 'reports.produksi_laminating_per_nomor_produksi';
        $connectionName = config("{$configPath}.database_connection");
        $procedure = (string) config("{$configPath}.stored_procedure", 'SPWps_LapProduksiLaminating');
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan produksi laminating per nomor produksi belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan produksi laminating per nomor produksi dikonfigurasi untuk SQL Server. '
                . 'Set PRODUKSI_LAMINATING_PER_NOMOR_PRODUKSI_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('PRODUKSI_LAMINATING_PER_NOMOR_PRODUKSI_REPORT_QUERY belum diisi.');
    ...
    private function detectDirection(array $row): ?string
    {
        ...
        if ($process !== '') {
            if (str_contains($process, 'finger') || $process === 'fj') {
                return 'input';
            }
            ...
            if ($process === 'lmt' || str_contains($process, 'lamin')) {
                return 'output';
            }
        }
