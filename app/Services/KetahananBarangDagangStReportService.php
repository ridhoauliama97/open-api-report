...
class KetahananBarangDagangStReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.ketahanan_barang_st.expected_columns', []);
    ...
    {
        $configKey = 'reports.ketahanan_barang_st';
        $connectionName = config("{$configKey}.database_connection");
        $procedure = (string) config("{$configKey}.stored_procedure", 'SP_LapKetahananBarangST');
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('Query manual laporan Ketahanan Barang Dagang ST belum diisi.');
