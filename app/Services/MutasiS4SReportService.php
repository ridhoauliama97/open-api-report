...
class MutasiS4SReportService
{
    ...
    public function fetchSubReport(string $startDate, string $endDate): array
    {
        ...
        $configuredColumns = config('reports.mutasi_s4s.expected_sub_columns', []);
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.mutasi_s4s.expected_columns', []);
    ...
    {
        $connectionName = config('reports.mutasi_s4s.database_connection');
        $procedure = (string) config(
            $isSubProcedure
                ? 'reports.mutasi_s4s.sub_stored_procedure'
                : 'reports.mutasi_s4s.stored_procedure'
        );
        $syntax = (string) config('reports.mutasi_s4s.call_syntax', 'exec');
        $customQuery = config(
            $isSubProcedure
                ? 'reports.mutasi_s4s.sub_query'
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException(
                $isSubProcedure
                    ...
                    : 'Stored procedure laporan mutasi s4s belum dikonfigurasi.',
            );
        }
