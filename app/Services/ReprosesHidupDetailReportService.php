...
class ReprosesHidupDetailReportService
{
    /**
    ...
    public function fetch(): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            $noReproses = trim((string) ($item['NoReproses'] ?? $item['No_Reproses'] ?? $item['No'] ?? ''));
            ...
            return [
                'NoReproses' => $noReproses !== '' ? $noReproses : null,
        ...
        usort($normalized, static function (array $a, array $b): int {
            ...
            return strcmp((string) ($a['NoReproses'] ?? ''), (string) ($b['NoReproses'] ?? ''));
        });
    ...
    public function healthCheck(): array
    {
        ...
        $expectedColumns = config('reports.reproses_hidup_detail.expected_columns', []);
    ...
    {
        $connectionName = config('reports.reproses_hidup_detail.database_connection');
        $procedure = (string) config('reports.reproses_hidup_detail.stored_procedure', 'SP_LapReprosesHidupDetail');
        $syntax = (string) config('reports.reproses_hidup_detail.call_syntax', 'exec');
        $customQuery = config('reports.reproses_hidup_detail.query');
        $parameterCount = (int) config('reports.reproses_hidup_detail.parameter_count', 0);

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan Reproses (Hidup) detail belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan Reproses (Hidup) detail dikonfigurasi untuk SQL Server. '
                . 'Set REPROSES_HIDUP_DETAIL_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'REPROSES_HIDUP_DETAIL_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan REPROSES_HIDUP_DETAIL_REPORT_CALL_SYNTAX=query.',
                );
