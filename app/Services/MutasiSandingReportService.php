...
class MutasiSandingReportService
{
    /**
    ...
    public function fetchSubReport(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.mutasi_sanding.expected_sub_columns', []);
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.mutasi_sanding.expected_columns', []);
        $expectedColumns = is_array($expectedColumns) ? array_values($expectedColumns) : [];
    ...
    {
        $connectionName = config('reports.mutasi_sanding.database_connection');
        $procedure = (string) config(
            $isSubProcedure
                ? 'reports.mutasi_sanding.sub_stored_procedure'
                : 'reports.mutasi_sanding.stored_procedure'
        );
        $syntax = (string) config('reports.mutasi_sanding.call_syntax', 'exec');
        $customQuery = config(
            $isSubProcedure
                ? 'reports.mutasi_sanding.sub_query'
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException(
                $isSubProcedure
                    ...
                    : 'Stored procedure laporan mutasi sanding belum dikonfigurasi.',
            );
        }
