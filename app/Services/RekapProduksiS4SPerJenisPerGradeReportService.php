...
class RekapProduksiS4SPerJenisPerGradeReportService
{
    /**
    ...
    public function fetch(string $startDate, string $endDate): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            return [
                ...
                'Output' => $this->toFloat($item['Output'] ?? null),
            ];
        }, $rows);

        // Sort by Jenis asc, then keep the "Jenis total row" (NamaGrade null) first, then NamaGrade asc.
        usort($normalized, static function (array $a, array $b): int {
            ...
            $aIsHeader = ($a['NamaGrade'] ?? null) === null || trim((string) ($a['NamaGrade'] ?? '')) === '';
            $bIsHeader = ($b['NamaGrade'] ?? null) === null || trim((string) ($b['NamaGrade'] ?? '')) === '';
            if ($aIsHeader !== $bIsHeader) {
                return $aIsHeader ? -1 : 1;
            }
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $required = ['Jenis', 'NamaGrade', 'ST', 'S4S', 'WIP', 'Reproses', 'Output'];
    ...
    {
        $connectionName = config('reports.rekap_produksi_s4s_per_jenis_per_grade.database_connection');
        $procedure = (string) config(
            'reports.rekap_produksi_s4s_per_jenis_per_grade.stored_procedure',
            'SP_LapRekapProduksiS4SPerJenisPerGrade',
        );
        $syntax = (string) config('reports.rekap_produksi_s4s_per_jenis_per_grade.call_syntax', 'exec');
