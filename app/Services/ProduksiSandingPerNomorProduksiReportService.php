...
class ProduksiSandingPerNomorProduksiReportService
{
    /**
    ...
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data produksi sanding tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.produksi_sanding_per_nomor_produksi.expected_columns', []);
    ...
    public function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        return [
            'meta' => [
                ...
                'stored_procedure' => (string) config('reports.produksi_sanding_per_nomor_produksi.stored_procedure', 'SPWps_LapProduksiSanding'),
                ...
                'nama_mesin' => $this->firstNonEmptyString($firstRow, ['NamaMesin', 'Mesin', 'Machine']) ?: 'SANDING',
                ...
                'output_label' => $outputLabel ?: 'SANDING',
            ],
    ...
    {
        $configPath = 'reports.produksi_sanding_per_nomor_produksi';
        $connectionName = config("{$configPath}.database_connection");
        $procedure = (string) config("{$configPath}.stored_procedure", 'SPWps_LapProduksiSanding');
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan produksi sanding per nomor produksi belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan produksi sanding per nomor produksi dikonfigurasi untuk SQL Server. '
                . 'Set PRODUKSI_SANDING_PER_NOMOR_PRODUKSI_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('PRODUKSI_SANDING_PER_NOMOR_PRODUKSI_REPORT_QUERY belum diisi.');
    ...
    private function detectDirection(array $row): ?string
    {
        ...
        if ($process !== '') {
            ...
            if (str_contains($process, 'sanding') || $process === 'sand') {
                return 'output';
            }
        }
