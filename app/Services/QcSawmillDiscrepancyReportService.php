...
class QcSawmillDiscrepancyReportService
{
    /**
    ...
    public function buildReportData(string $startDate, string $endDate): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data QC Sawmill - Discrepancy tidak ditemukan untuk rentang tanggal yang dipilih.');
        }
        ...
        foreach ($rows as $row) {
            ...
            if (! isset($groups[$groupKey])) {
                $groups[$groupKey] = [
                    ...
                    '_sum_deviation_tebal' => 0.0,
                    '_sum_deviation_lebar' => 0.0,
                ];
            }

            $groups[$groupKey]['summary']['total_rows']++;
            $groups[$groupKey]['_sum_deviation_tebal'] += (float) ($row['DeviationTebal'] ?? 0);
            $groups[$groupKey]['_sum_deviation_lebar'] += (float) ($row['DeviationLebar'] ?? 0);
            ...
            $row['DisplayNo'] = (int) ($groups[$groupKey]['summary']['total_rows'] ?? 0);
            $groups[$groupKey]['rows'][] = $row;
            $groups[$groupKey]['summary']['total_discrepancy']++;
        }

        $groups = array_values($groups);
        foreach ($groups as &$group) {
            $totalRows = (int) ($group['summary']['total_rows'] ?? 0);
            $sumDeviationTebal = (float) ($group['_sum_deviation_tebal'] ?? 0);
            $sumDeviationLebar = (float) ($group['_sum_deviation_lebar'] ?? 0);
            $totalAccurate = (int) ($group['summary']['total_accurate'] ?? 0);

            $group['summary'] = [
                'total_rows' => $totalRows,
                'total_accurate' => $totalAccurate,
                'total_discrepancy' => (int) ($group['summary']['total_discrepancy'] ?? 0),
            ...
            unset($group['_sum_deviation_tebal'], $group['_sum_deviation_lebar']);
        }
        unset($group);
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.qc_sawmill_discrepancy.expected_columns', []);
    ...
    {
        $configKey = 'reports.qc_sawmill_discrepancy';
        $connectionName = config("{$configKey}.database_connection");
        $procedure = (string) config("{$configKey}.stored_procedure", 'SP_LapQCSawmillDescr');
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('Query manual laporan QC Sawmill - Discrepancy belum diisi.');
