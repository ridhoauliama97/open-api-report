...
class KetahananBarangDagangCrossCutAkhirReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.ketahanan_barang_cross_cut_akhir.expected_columns', []);
    ...
    {
        $configKey = 'reports.ketahanan_barang_cross_cut_akhir';
        $connectionName = config("{$configKey}.database_connection");
        $procedure = (string) config("{$configKey}.stored_procedure", 'SP_LapKetahananBarangCCAkhir');
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('Query manual laporan Ketahanan Barang Dagang CCAkhir belum diisi.');
