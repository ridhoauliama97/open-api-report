...
class RekapProduksiBrokerReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.pps_rekap_produksi_broker.expected_columns', []);
    ...
    {
        $configPath = 'reports.pps_rekap_produksi_broker';
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan PPS Rekap Produksi Broker belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS Rekap Produksi Broker dikonfigurasi untuk SQL Server. '
                . 'Set PPS_REKAP_PRODUKSI_BROKER_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_REKAP_PRODUKSI_BROKER_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan PPS_REKAP_PRODUKSI_BROKER_REPORT_CALL_SYNTAX=query.',
                );
