...
class HasilProduksiHarianHotStampingProduksiReportService
{
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data hot stamping produksi tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.pps_hot_stamping_produksi_harian.expected_columns', []);
    ...
    {
        $connectionName = config('reports.pps_hot_stamping_produksi_harian.database_connection');
    ...
    private function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        for ($index = 0; $index < $detailCount; $index++) {
            ...
            $detailRows[] = [
                ...
                'output_jumlah_label' => $outputs[$index]['jumlah_label'] ?? '',
        ...
        return [
            ...
            'approvals' => [
                ...
                'ka_regu_hot_stamping' => (string) ($firstRow['CheckBy1'] ?? ''),
                'ka_div_hot_stamping' => (string) ($firstRow['CheckBy2'] ?? ''),
    ...
    {
        $configPath = 'reports.pps_hot_stamping_produksi_harian';
        ...
        if ($procedure === '') {
            throw new RuntimeException('Stored procedure laporan PPS Hot Stamping Produksi Harian belum dikonfigurasi.');
        }
