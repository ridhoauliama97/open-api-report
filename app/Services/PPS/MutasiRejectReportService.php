...
class MutasiRejectReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.pps_mutasi_reject.expected_columns', []);
    ...
    {
        $configPath = 'reports.pps_mutasi_reject';
        ...
        $singleParameterName = (string) config("{$configPath}.single_parameter_name", 'EndDate');

        if ($procedure === '' && ! is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan PPS Mutasi Reject belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS Mutasi Reject dikonfigurasi untuk SQL Server. '
                .'Set PPS_MUTASI_REJECT_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_MUTASI_REJECT_REPORT_QUERY belum diisi. '
                    .'Isi query manual jika menggunakan PPS_MUTASI_REJECT_REPORT_CALL_SYNTAX=query.',
                );
    ...
    private function buildBindings(
        ...
        return match ($parameterCount) {
            0 => [],
            1 => [strtolower($singleParameterName) === 'startdate' ? $startDate : $endDate],
