...
class HasilProduksiHarianPasangKunciProduksiReportService
{
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data pasang kunci produksi tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.pps_pasang_kunci_produksi_harian.expected_columns', []);
    ...
    {
        $connectionName = config('reports.pps_pasang_kunci_produksi_harian.database_connection');
    ...
    private function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        return [
            ...
            'approvals' => [
                ...
                'ka_regu_pasang_kunci' => (string) ($firstRow['CheckBy1'] ?? ''),
                'ka_div_pasang_kunci' => (string) ($firstRow['CheckBy2'] ?? ''),
    ...
    {
        $configPath = 'reports.pps_pasang_kunci_produksi_harian';
        ...
        if ($procedure === '') {
            throw new RuntimeException('Stored procedure laporan PPS Pasang Kunci Produksi Harian belum dikonfigurasi.');
        }
