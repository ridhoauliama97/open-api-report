...
class StockWashingV2ReportService
{
    ...
    public function healthCheck(string $startDate, string $endDate, string $warehouse): array
    {
        ...
        $expectedColumns = config('reports.pps_stock_washing_v2.expected_columns', []);
    ...
    {
        $configPath = 'reports.pps_stock_washing_v2';
        ...
        if ($procedure === '' && ! is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan PPS Stock Washing V2 belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS Stock Washing V2 dikonfigurasi untuk SQL Server. '
                .'Set PPS_STOCK_WASHING_V2_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_STOCK_WASHING_V2_REPORT_QUERY belum diisi. '
                    .'Isi query manual jika menggunakan PPS_STOCK_WASHING_V2_REPORT_CALL_SYNTAX=query.',
                );
