...
class StockFurnitureWipV2ReportService
{
    ...
    public function healthCheck(string $startDate, string $endDate, string $warehouse): array
    {
        ...
        $expectedColumns = config('reports.pps_stock_furniture_wip_v2.expected_columns', []);
    ...
    {
        $configPath = 'reports.pps_stock_furniture_wip_v2';
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan PPS Stock Furniture WIP V2 belum dikonfigurasi.');
        }

        $connection = DB::connection($connectionName ?: null);
        $driver = $connection->getDriverName();
        $bindings = $this->buildBindings($parameterCount, $singleParameterName, $startDate, $endDate, $warehouse);

        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS Stock Furniture WIP V2 dikonfigurasi untuk SQL Server. '
                . 'Set PPS_STOCK_FURNITURE_WIP_V2_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_STOCK_FURNITURE_WIP_V2_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan PPS_STOCK_FURNITURE_WIP_V2_REPORT_CALL_SYNTAX=query.',
                );
