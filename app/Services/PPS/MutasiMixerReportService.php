...
class MutasiMixerReportService
{
    /**
    ...
    {
        $connectionName = config('reports.pps_mutasi_mixer.database_connection');
        $procedureConfig = $procedureType === 'sub'
            ? 'reports.pps_mutasi_mixer.sub_stored_procedure'
            : 'reports.pps_mutasi_mixer.stored_procedure';
        $queryConfig = $procedureType === 'sub'
            ? 'reports.pps_mutasi_mixer.sub_query'
            : 'reports.pps_mutasi_mixer.query';
        $procedure = (string) config($procedureConfig);
        $syntax = (string) config('reports.pps_mutasi_mixer.call_syntax', 'exec');
        $customQuery = config($queryConfig);

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException(
                $procedureType === 'sub'
                    ? 'Stored procedure sub laporan PPS mutasi mixer belum dikonfigurasi.'
                    : 'Stored procedure laporan PPS mutasi mixer belum dikonfigurasi.'
            );
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS mutasi mixer dikonfigurasi untuk SQL Server. '
                . 'Set PPS_MUTASI_MIXER_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_MUTASI_MIXER_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan PPS_MUTASI_MIXER_REPORT_CALL_SYNTAX=query.',
                );
    ...
    private function normalizeReportRows(array $rows): array
    {
        ...
        if (!empty($missingColumns)) {
            throw new RuntimeException(
                'Output SP_PPSLapMutasiMixer tidak sesuai. Kolom tidak ditemukan: '
    ...
    private function normalizeSubReportRows(array $rows): array
    {
        ...
        if (!empty($missingColumns)) {
            throw new RuntimeException(
                'Output SP_PPSLapSubMutasiMixer tidak sesuai. Kolom tidak ditemukan: '
                . implode(', ', $missingColumns),
            );
        }
