...
class HasilProduksiHarianGilinganProduksiReportService
{
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data gilingan produksi tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.pps_gilingan_produksi_harian.expected_columns', []);
    ...
    {
        $connectionName = config('reports.pps_gilingan_produksi_harian.database_connection');
    ...
    private function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $item = [
                'no_label' => $this->stringValue($row, ['NoLabel', 'NomorLabel', 'NoGilingan', 'NoBarang']),
        ...
        return [
            ...
            'approvals' => [
                ...
                'ka_regu_gilingan' => (string) ($firstRow['CheckBy1'] ?? ''),
                'ka_div_gilingan' => (string) ($firstRow['CheckBy2'] ?? ''),
    ...
    {
        $configPath = 'reports.pps_gilingan_produksi_harian';
        ...
        if ($procedure === '') {
            throw new RuntimeException('Stored procedure laporan PPS Gilingan Produksi Harian belum dikonfigurasi.');
        }
    ...
    {
        $aliasMap = [
            'nomor_label' => ['NoLabel', 'NomorLabel', 'NoGilingan', 'NoBarang'],
