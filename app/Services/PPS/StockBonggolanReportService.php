...
class StockBonggolanReportService
{
    ...
    public function healthCheck(string $startDate, string $endDate, string $warehouseName): array
    {
        ...
        $expectedColumns = config('reports.pps_stock_bonggolan.expected_columns', []);
    ...
    {
        $configPath = 'reports.pps_stock_bonggolan';
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan PPS Stock Bonggolan belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS Stock Bonggolan dikonfigurasi untuk SQL Server. '
                . 'Set PPS_STOCK_BONGGOLAN_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_STOCK_BONGGOLAN_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan PPS_STOCK_BONGGOLAN_REPORT_CALL_SYNTAX=query.',
                );
