...
class RekapProduksiPasangKunciFwipReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.pps_rekap_produksi_pasang_kunci_fwip.expected_columns', []);
    ...
    {
        $configPath = 'reports.pps_rekap_produksi_pasang_kunci_fwip';
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan PPS Rekap Produksi Inject belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan PPS Rekap Produksi Inject dikonfigurasi untuk SQL Server. '
                . 'Set PPS_REKAP_PRODUKSI_PASANG_KUNCI_FWIP_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'PPS_REKAP_PRODUKSI_PASANG_KUNCI_FWIP_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan PPS_REKAP_PRODUKSI_PASANG_KUNCI_FWIP_REPORT_CALL_SYNTAX=query.',
                );
