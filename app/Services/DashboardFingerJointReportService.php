...
class DashboardFingerJointReportService
{
    ...
    public function buildReportData(string $startDate, string $endDate): array
    {
        ...
        if (
            ...
            throw new RuntimeException(
                'Kolom wajib tidak ditemukan di hasil SPWps_LapDashboardFJ. '
                . 'Butuh kolom tanggal, jenis, grade, masuk, dan keluar. '
                . 'Kolom terdeteksi: ' . implode(', ', $detectedColumns),
            );
        }

        $orderedColumns = config('reports.dashboard_finger_joint.column_order', []);
        $orderedColumns = is_array($orderedColumns) ? array_values($orderedColumns) : [];
        $ctrDivisor = (float) config('reports.dashboard_finger_joint.ctr_divisor', 65);
        if ($ctrDivisor <= 0) {
            $ctrDivisor = 65.0;
        }
    ...
    private function resolveColumns(array $rows): array
    {
        ...
        return [
            ...
            'masuk' => $this->findMatchingKey($keys, ['fjmasuk', 'masukall', 'masuk', 'inflow', 'debit']),
            'keluar' => $this->findMatchingKey($keys, ['keluarall', 'fjkeluar', 'fjjual', 'jual', 'keluar', 'outflow', 'kredit']),
            's_akhir' => $this->findMatchingKey($keys, ['fjakhir', 'akhir', 's_akhir', 'stokakhir', 'saldoakhir']),
    ...
    {
        $connectionName = config('reports.dashboard_finger_joint.database_connection');
        $procedure = (string) config('reports.dashboard_finger_joint.stored_procedure');
        $syntax = (string) config('reports.dashboard_finger_joint.call_syntax', 'exec');
        $customQuery = config('reports.dashboard_finger_joint.query');
        $parameterCount = (int) config('reports.dashboard_finger_joint.parameter_count', 2);
        $parameterCount = max(0, min(2, $parameterCount));

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan dashboard finger joint belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan dashboard finger joint dikonfigurasi untuk SQL Server. '
                . 'Set DASHBOARD_FINGER_JOINT_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'DASHBOARD_FINGER_JOINT_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan DASHBOARD_FINGER_JOINT_REPORT_CALL_SYNTAX=query.',
                );
