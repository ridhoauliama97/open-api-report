...
class PenerimaanKayuBulatExtKgReportService
{
    /**
    ...
    {
        $configKey = 'reports.penerimaan_kayu_bulat_extkg';
        $connection = DB::connection(config("{$configKey}.database_connection"));
        $procedure = (string) config("{$configKey}.stored_procedure", 'SP_PenKBOutTon_KG');
        ...
        if ($syntax !== 'exec') {
            throw new RuntimeException('Laporan penerimaan kayu bulat ExtKG saat ini hanya mendukung call syntax exec.');
        }
    ...
    public function healthCheck(string $noKayuBulat): array
    {
        ...
        $expectedColumns = config('reports.penerimaan_kayu_bulat_extkg.expected_columns', []);
