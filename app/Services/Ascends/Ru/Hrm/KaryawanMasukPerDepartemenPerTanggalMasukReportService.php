...
class KaryawanMasukPerDepartemenPerTanggalMasukReportService
{
    private const TITLE = 'Laporan Karyawan Masuk Per Departemen Per Tanggal Masuk (RU)';
    ...
    {
        $reportData = $this->xmlDataSourceService->loadSubReport('RU', 'hrm', 'karyawan_masuk_per_departemen_per_tanggal_masuk');
    ...
    {
        return trim((string) ($row['Tanggal Masuk'] ?? '')) !== ''
            && ! str_starts_with(strtoupper(trim((string) ($row['Kode Karyawan'] ?? ''))), 'SPECIAL');
    }
