...
class PengirimanPerKategoriHarianReportService
{
    private const TITLE = 'Laporan Pengiriman Per Kategori (Harian)';
    ...
    private function determineKeterangan(string $itemName): string
    {
        ...
        if (str_contains($upper, 'PROMO MORE')) {
            return 'NOT';
        }
        if (str_contains($upper, 'PROMO 6 BH')) {
            return 'NOT';
        }
    ...
    private function buildCategories(array $rows, array $dayNumbers): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $keterangan = $this->determineKeterangan($itemName);
            if ($keterangan !== 'TAMPIL') {
                continue;
            }
        ...
        foreach (self::CATEGORY_ORDER as $key) {
            ...
            if ($key === 'plastik_kabinet_1') {
                $categories[] = $this->buildKabinet1Category($categorized[$key], $dayNumbers);
            } else {
    ...
    private function buildKabinet1Category(array $rows, array $dayNumbers): array
    {
        // Group by Grp first, then by item within each Grp
        $grpGroups = [];
        foreach ($rows as $row) {
            $itemName = (string) ($row['ItemName'] ?? '');
        ...
        // Sort groups: 3TX6P, 4TX8P, then others
    ...
    {
        // Group rows by ItemCode+ItemName, accumulating qty per day
        $itemMap = [];
        foreach ($rows as $row) {
            $itemName = (string) ($row['ItemName'] ?? '');
