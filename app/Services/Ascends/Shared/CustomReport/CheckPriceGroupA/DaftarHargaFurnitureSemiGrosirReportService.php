...
class DaftarHargaFurnitureSemiGrosirReportService
{
    private const TITLE = 'DAFTAR HARGA FURNITURE SEMI GROSIR';
    ...
    private function processItems(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            // Track max base price for fallback
            if ($price > $groupedItems[$key]['base_price']) {
                $groupedItems[$key]['base_price'] = $price;
            }

            // Harga Konsumen = Price at 01. Harga Retail specifically
            if (str_contains($priceLevel, '01. Harga Retail')) {
                $groupedItems[$key]['harga_konsumen'] = $price;
            }

            // Semi Grosir = PriceAfterDisc at 02. Harga Semi Grosir / Conversion
            if (str_contains($priceLevel, '02. Harga Semi Grosir')) {
                $groupedItems[$key]['semi_grosir'] = $conversion > 0 ? $afterDisc / $conversion : 0.0;
                // Override per_dus from Semi Grosir level (matches Crystal Report behavior)
                $groupedItems[$key]['per_dus'] = (float) ($row['PerDus'] ?? $groupedItems[$key]['per_dus']);
            }
        }

        // Apply fallback for semi_grosir: use base_price when no semi level data
        foreach ($groupedItems as &$item) {
            if ($item['semi_grosir'] == 0.0 && $item['base_price'] > 0.0) {
                $item['semi_grosir'] = $item['base_price'];
            }
        }
        unset($item);
