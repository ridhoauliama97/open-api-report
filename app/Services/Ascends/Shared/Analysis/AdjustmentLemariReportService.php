...
class AdjustmentLemariReportService
{
    private const TITLE = 'Laporan Adjustment Lemari';
    ...
    private function parseXml(string $xmlContents, string $sourceLabel): array
    {
        ...
        while ($reader->read()) {
            ...
            $rows[] = [
                ...
                'customer_name' => trim((string) ($node->Customer_x0020_Name ?? '')),
        ...
        if ($rows === []) {
            throw new RuntimeException('Data adjustment lemari tidak ditemukan di XML.');
        }
    ...
    private static function publicRow(array $row): array
    {
        ...
        $uom = (string) ($row['uom'] ?? '');
        $jumlah = number_format($quantity, 0, '.', ',');

        return [
            'No. Memo' => (string) ($row['memo_number'] ?? ''),
            'Konsumen/Pelanggan' => (string) ($row['customer_name'] ?? ''),
            'Tanggal Penyesuaian' => (string) ($row['adjustment_date_display'] ?? ''),
            'Nama Barang' => (string) ($row['item_name'] ?? ''),
            'Jumlah' => $jumlah,
            'UOM' => $uom,
            'Penyesuaian Nilai' => number_format((float) ($row['adjusted_value'] ?? 0), 2, '.', ','),
        ];
    }
