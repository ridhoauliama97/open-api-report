...
class AktifitasStockRuReportService
{
    private const TITLE = 'Laporan Ringkasan Valuasi Persediaan';
    ...
    private const SUMMARY_ONLY_CATEGORIES = [
        'PERS. LAINNYA' => true,
        'PERSEDIAAN LAINNYA' => true,
    ];
    ...
    private function parseXml(string $xmlContents, string $sourceLabel): array
    {
        ...
        while ($reader->read()) {
            ...
            $prdQt = $assmAddlOut + $assmGood;
            $prdRp = $assmAddlOutValue + $assmGoodValue + $adjDbValue + $salesReturnValue;
            $qtyPenjualan = $sales + ($salesReturn * -1) + $gdn;
            $valuePenjualan = $salesValue + ($salesReturnValue * -1) + $gdnValue;
            $proOutQt = $assmMaterial + $usage;
        ...
        foreach ($rows as $row) {
            ...
            if (! isset($categoryGroups[$cat])) {
                $categoryGroups[$cat] = [
                    ...
                    'is_summary' => isset(self::SUMMARY_ONLY_CATEGORIES[$cat]),
                ];
            }

            if ($categoryGroups[$cat]['is_summary']) {
                $categoryGroups[$cat]['totals'] = $this->accumulateTotals($categoryGroups[$cat]['totals'], $row);
                $rowHpp = ($row['ending'] > 0) ? $row['ending_value'] / $row['ending'] : 0;
                $categoryGroups[$cat]['totals']['hpp'] += $rowHpp;
