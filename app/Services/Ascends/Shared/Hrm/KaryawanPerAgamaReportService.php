...
class KaryawanPerAgamaReportService
{
    ...
    {
        $reportData = $this->xmlDataSourceService->loadSubReport('RU', 'hrm', 'karyawan_per_agama');
    ...
    private function shapeReportData(array $reportData, string $sourceLabel): array
    {
        ...
        $grandSummary['religion'] = self::countWithPercent($rows, 'Agama', self::religionLabels($rows));
     ...
     * @return array<string, string>
     */
    private static function religionLabels(array $rows): array
    {
        $labels = [];
