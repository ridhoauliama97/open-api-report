...
class KehadiranKruStickReportService
{
    private const TARGET_JOB_TITLE = 'KRU STICK BORONGAN';

    private const TARGET_WORKGROUP = 'BORONGAN STICK';
    ...
    public function buildReportDataFromXml(string $xmlContents, string $sourceLabel = 'request xml payload', array $filters = []): array
    {
        ...
        if ($periodRows === []) {
            throw new RuntimeException('Data Kehadiran Kru Stick tidak ditemukan pada periode yang dipilih.');
        }
        ...
        if ($employees === []) {
            throw new RuntimeException('Data Kehadiran Kru Stick tidak ditemukan pada XML/periode yang dipilih.');
        }

        return [
            'title' => 'Laporan Kehadiran Kru Stick',
    ...
    private function resolveReportPeriod(array $filters, array $rows): array
    {
        ...
        $dates = array_values(array_filter(array_map(
            ...
            array_filter($rows, fn (array $row): bool => $this->isKruStickRow($row))
        )));
    ...
    private function isKruStickRow(array $row): bool
    {
        return strtoupper(trim((string) ($row['Job Title'] ?? ''))) === self::TARGET_JOB_TITLE
            && strtoupper(trim((string) ($row['Workgroup'] ?? ''))) === self::TARGET_WORKGROUP;
    }
