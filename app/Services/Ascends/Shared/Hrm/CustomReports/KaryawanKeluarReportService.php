...
class KaryawanKeluarReportService
{
    ...
    public function buildReportDataFromXml(string $xmlContents, string $sourceLabel = 'request xml payload', array $filters = []): array
    {
        ...
        return [
            'title' => 'Laporan Karyawan Keluar Per Departemen Per Tanggal Keluar',
            ...
            'headerTitle' => 'Laporan Karyawan Keluar Per Departemen Per Tanggal Keluar',
            ...
            'headers' => ['No', 'Nama', 'L/P', 'Jabatan', 'Status', 'Level', 'Tanggal Masuk', 'Tanggal Keluar', 'Masa Kerja', 'Alasan Keluar', 'Hasil'],
    ...
    {
        $retirementDate = $this->parseDate((string) ($row['RetirementDate'] ?? ''));
        if ($retirementDate === null) {
            return false;
        }

        if ($startDate !== null && $retirementDate->lessThan($startDate->startOfDay())) {
            return false;
        }

        if ($endDate !== null && $retirementDate->greaterThan($endDate->endOfDay())) {
            return false;
        }
    ...
    private function resolveReportDate(array $filters, array $rows): string
    {
        ...
        $dates = array_filter(array_map(
            fn (array $row): ?Carbon => $this->parseDate((string) ($row['RetirementDate'] ?? '')),
            $rows
        ));
        rsort($dates);
    ...
    private function shapeRow(array $row): array
    {
        ...
        return [
            ...
            'Tanggal Keluar' => $this->formatDate($retirementDate),
            'Tanggal Keluar Sort' => $this->dateSortKey($retirementDate),
            'Masa Kerja' => $this->computeMasaKerja($joinDate, $retirementDate),
            'Alasan Keluar' => (string) ($row['RetirementReason'] ?? ''),
            'Hasil' => (string) ($row['Hasil'] ?? ''),
            'Departemen' => trim((string) ($row['DepartmentName'] ?? '')),
        ];
    }

    private function computeMasaKerja(string $joinDateStr, string $retirementDateStr): string
    {
        $joinDate = $this->parseDate($joinDateStr);
        $retirementDate = $this->parseDate($retirementDateStr);

        if ($joinDate === null || $retirementDate === null) {
            return '';
        }
