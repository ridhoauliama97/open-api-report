...
class ArusKasGsuReportService
{
    ...
    private function tampilKas(string $ac1): bool
    {
        ...
        $piutang9 = ['111.200.1', '111.200.2', '111.200.3', '111.300.2'];
        if (in_array($left9, $piutang9, true)) {
            return true;
        }

        $persediaan9 = ['111.400.1', '111.400.2', '111.400.3', '111.400.4', '111.400.5', '111.400.9'];
        if (in_array($left9, $persediaan9, true)) {
            return true;
        }

        $specificPl = ['112.100.101', '112.100.103', '112.100.199'];
        if (in_array($ac1, $specificPl, true)) {
            return true;
        }
    ...
    private function groupItemsBySection(array $filtered, string $periodStart, string $periodEnd): array
    {
        ...
        $ac1Sections['1.3'] = ['111.200', '111.300', '112.100'];
        $ac1Sections['1.4'] = ['111.400'];
        $ac1Sections['1.5'] = ['112.200', '112.300'];
        $ac1Sections['1.6'] = ['211.100'];
        $ac1Sections['1.7'] = ['211.400'];
        $ac1Sections['1.8'] = ['212.100', '211.200', '211.300', '211.500'];
        $ac1Sections['2.1'] = ['121.100', '121.102'];
        $ac1Sections['3.1'] = ['222.000'];

        $sections = [];
        $sectionCodes = ['1.3', '1.4', '1.5', '1.6', '1.7', '1.8', '2.1', '3.1'];

        $sectionLabels = [
            '1.3' => 'Penurunan (Kenaikan) Piutang Dagang',
            '1.4' => 'Penurunan (Kenaikan) Persediaan',
            '1.5' => 'Penurunan (Kenaikan) Biaya Dibayar Dimuka',
            '1.6' => 'Penurunan (Kenaikan) Hutang Dagang',
            '1.7' => 'Kenaikan (Penurunan) Kewajiban Yang Masih Harus Dibayar',
            '1.8' => 'Kenaikan (Penurunan) Kewajiban Lancar Lainnya',
            '2.1' => 'Penurunan (Kenaikan) Aktiva Tetap',
            '3.1' => 'Kenaikan (Penurunan) Hutang Jangka Panjang',
        ];

        $totalLabels = [
            '1.3' => 'Total Arus Kas Dari Penurunan (Kenaikan) Piutang Dagang',
            '1.4' => 'Total Arus Kas Dari Penurunan (Kenaikan) Persediaan',
            '1.5' => 'Total Arus Kas Dari Penurunan (Kenaikan) Biaya Dibayar Dimuka',
            '1.6' => 'Total Arus Kas Dari Penurunan (Kenaikan) Hutang Dagang',
            '1.7' => 'Total Arus Kas Dari Kenaikan (Penurunan) Kewajiban Yang Masih Harus Dibayar',
            '1.8' => 'Total Arus Kas Dari Kenaikan (Penurunan) Kewajiban Lancar Lainnya',
            '2.1' => 'Total Arus Kas Dari Penurunan (Kenaikan) Aktiva Tetap',
            '3.1' => 'Total Arus Kas Dari Kenaikan (Penurunan) Hutang Jangka Panjang',
        ];

        $liabilitySections = ['1.6', '1.7', '1.8', '3.1'];
    ...
    private function buildSections(array $filtered, array $allRows, string $periodStart, string $periodEnd): array
    {
        ...
        $operasiItems[] = [
            'type' => 'sub_header',
            'label' => '1.2 Rekonsel By Penyusutan',
            'code' => '1.2',
            'amount' => $penyusutan,
        ];

        $subSections = ['1.3', '1.4', '1.5', '1.6', '1.7', '1.8'];
        ...
        $pendanaanTotal = 0.0;
        foreach ($pendanaanSubSections as $s) {
            $pendanaanTotal += $sectionGroups[$s]['total'] ?? 0;
        }
    ...
    {
        $cashPrefixes = ['111.101', '111.102', '111.105'];
        $total = 0.0;
        $processed = [];
