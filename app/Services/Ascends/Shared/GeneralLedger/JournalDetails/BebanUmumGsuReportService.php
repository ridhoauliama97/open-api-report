...
class BebanUmumGsuReportService
{
    private const TITLE = 'Laporan Beban Umum';
    ...
    public function buildReportDataFromXml(string $xmlContents, string $sourceLabel = 'request xml payload', array $filters = []): array
    {
        ...
        $summary = $groups;
        usort($summary, static fn (array $a, array $b): int => $b['subtotal'] <=> $a['subtotal']);
    ...
    {
        if ($accountCode >= '721.000.000' && $accountCode <= '721.000.999') {
            return true;
        }

        $gsuSpecific = ['721.000.201A', '721.000.201B', '721.000.201C', '721.000.201D', '721.000.201E'];
        foreach ($gsuSpecific as $prefix) {
    ...
    private function computeRows(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $result[] = [
                ...
                'description' => $this->computeDescription($row),
                'amount_db' => $amountDb,
                'amount_cr' => $amountCr,
    ...
    private function computeDescription(array $row): string
    {
        $voucherNumber = (string) ($row['Voucher Number'] ?? '');
        $description = (string) ($row['Description'] ?? '');

        if (in_array($voucherNumber, ['RU/IU/21/03/0045', 'RU/IU/21/03/0046'], true)) {
