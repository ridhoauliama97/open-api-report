...
class RekapProduksiMouldingPerJenisPerGradeReportService
{
    /**
    ...
    public function fetch(string $startDate, string $endDate): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            return [
                ...
                'InS4S' => $this->toFloat($item['S4S'] ?? $item['InS4S'] ?? null),
                'InFJ' => $this->toFloat($item['FJ'] ?? $item['InFJ'] ?? null),
                'InMoulding' => $this->toFloat($item['Moulding'] ?? $item['InMoulding'] ?? null),
                'InLaminating' => $this->toFloat($item['Laminating'] ?? $item['InLaminating'] ?? null),
                ...
                'OutReproses' => $this->toFloat($item['OutputReproses'] ?? $item['OutReproses'] ?? null),
            ];
        }, $rows);
    ...
    {
        $connectionName = config('reports.rekap_produksi_moulding_per_jenis_per_grade.database_connection');
        $procedure = (string) config(
            'reports.rekap_produksi_moulding_per_jenis_per_grade.stored_procedure',
            'SP_LapRekapProduksiMouldingPerJenisPerGrade',
        );
        $syntax = (string) config('reports.rekap_produksi_moulding_per_jenis_per_grade.call_syntax', 'exec');
        $customQuery = config('reports.rekap_produksi_moulding_per_jenis_per_grade.query');
        $parameterCount = (int) config('reports.rekap_produksi_moulding_per_jenis_per_grade.parameter_count', 2);

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan rekap produksi Moulding per-jenis dan per-grade belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan rekap produksi Moulding per-jenis dan per-grade dikonfigurasi untuk SQL Server. '
                . 'Set REKAP_PRODUKSI_MOULDING_PER_JENIS_PER_GRADE_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'REKAP_PRODUKSI_MOULDING_PER_JENIS_PER_GRADE_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan REKAP_PRODUKSI_MOULDING_PER_JENIS_PER_GRADE_REPORT_CALL_SYNTAX=query.',
                );
