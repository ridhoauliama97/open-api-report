...
class UmurMouldingDetailReportService
{
    ...
    public function healthCheck(array $parameters): array
    {
        ...
        $expectedColumns = config('reports.umur_moulding_detail.expected_columns', []);
    ...
    {
        $connectionName = config('reports.umur_moulding_detail.database_connection');
        $procedure = (string) config('reports.umur_moulding_detail.stored_procedure', 'SP_LapUmurMoulding');
        $syntax = (string) config('reports.umur_moulding_detail.call_syntax', 'exec');
