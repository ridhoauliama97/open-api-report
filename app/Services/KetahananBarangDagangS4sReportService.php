...
class KetahananBarangDagangS4sReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.ketahanan_barang_s4s.expected_columns', []);
    ...
    private function fetch(string $startDate, string $endDate): array
    {
        ...
        foreach ($raw as $row) {
            ...
            // Pada contoh report perusahaan, Avg Penjualan tampak sama dengan Penjualan (bukan dibagi hari).
    ...
    {
        $configKey = 'reports.ketahanan_barang_s4s';
        $connectionName = config("{$configKey}.database_connection");
        $procedure = (string) config("{$configKey}.stored_procedure", 'SP_LapKetahananBarangS4S');
        $syntax = (string) config("{$configKey}.call_syntax", 'exec');
        $customQuery = config("{$configKey}.query");
        $parameterCount = (int) config("{$configKey}.parameter_count", 2);

        if ($parameterCount !== 2) {
            throw new RuntimeException('Jumlah parameter laporan Ketahanan Barang Dagang S4S harus 2 (Tanggal Awal dan Tanggal Akhir).');
        }

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan Ketahanan Barang Dagang S4S belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan Ketahanan Barang Dagang S4S dikonfigurasi untuk SQL Server. '
                . 'Set KETAHANAN_BARANG_S4S_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('Query manual laporan Ketahanan Barang Dagang S4S belum diisi.');
