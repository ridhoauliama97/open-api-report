...
class ProduksiPackingPerNomorProduksiReportService
{
    /**
    ...
    public function fetch(string $noProduksi): array
    {
        ...
        if ($rows === []) {
            throw new RuntimeException('Data produksi packing tidak ditemukan untuk nomor produksi tersebut.');
        }
    ...
    public function healthCheck(string $noProduksi): array
    {
        ...
        $expectedColumns = config('reports.produksi_packing_per_nomor_produksi.expected_columns', []);
    ...
    public function buildReportData(string $noProduksi, array $rows): array
    {
        ...
        return [
            'meta' => [
                ...
                'stored_procedure' => (string) config('reports.produksi_packing_per_nomor_produksi.stored_procedure', 'SPWps_LapProduksiPacking'),
                ...
                'nama_mesin' => $this->firstNonEmptyString($firstRow, ['NamaMesin', 'Mesin', 'Machine']) ?: 'PACKING',
                ...
                'input_label' => $inputLabel ?: 'MOULDING',
                'output_label' => $outputLabel ?: 'PACKING',
            ],
    ...
    {
        $configPath = 'reports.produksi_packing_per_nomor_produksi';
        $connectionName = config("{$configPath}.database_connection");
        $procedure = (string) config("{$configPath}.stored_procedure", 'SPWps_LapProduksiPacking');
        ...
        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan produksi packing per nomor produksi belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan produksi packing per nomor produksi dikonfigurasi untuk SQL Server. '
                . 'Set PRODUKSI_PACKING_PER_NOMOR_PRODUKSI_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException('PRODUKSI_PACKING_PER_NOMOR_PRODUKSI_REPORT_QUERY belum diisi.');
    ...
    private function detectDirection(array $row): ?string
    {
        ...
        if ($process !== '') {
            ...
            if (str_contains($process, 'packing') || str_contains($process, 'pack') || $process === 'pkg') {
                return 'output';
            }
        }
