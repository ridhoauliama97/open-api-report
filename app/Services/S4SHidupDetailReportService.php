...
class S4SHidupDetailReportService
{
    /**
    ...
    public function fetch(): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            // Try to be resilient to slight column naming differences.
            $noS4S = (string) ($item['NoS4S'] ?? $item['NoS4s'] ?? $item['No_S4S'] ?? '');
            $dateCreate = (string) ($item['DateCreate'] ?? $item['TglCreate'] ?? $item['TglCreateS4S'] ?? '');
            ...
            $kubik = $this->toFloat($item['Kubik'] ?? $item['M3'] ?? $item['M3Kubik'] ?? null);
            $lokasi = trim((string) ($item['Lokasi'] ?? $item['IdLokasi'] ?? $item['IDLokasi'] ?? $item['Lok'] ?? ''));

            return [
                'NoS4S' => $noS4S !== '' ? $noS4S : null,
        ...
        // Stable sorting (DateCreate DESC, then NoS4S ASC).
    ...
    public function healthCheck(): array
    {
        ...
        $expectedColumns = config('reports.s4s_hidup_detail.expected_columns', []);
    ...
    {
        $connectionName = config('reports.s4s_hidup_detail.database_connection');
        $procedure = (string) config('reports.s4s_hidup_detail.stored_procedure', 'SP_LapS4SHidupDetail');
        $syntax = (string) config('reports.s4s_hidup_detail.call_syntax', 'exec');
