...
class UmurCrossCutAkhirDetailReportService
{
    ...
    public function healthCheck(array $parameters): array
    {
        ...
        $expectedColumns = config('reports.umur_cross_cut_akhir_detail.expected_columns', []);
    ...
    {
        $connectionName = config('reports.umur_cross_cut_akhir_detail.database_connection');
        $procedure = (string) config('reports.umur_cross_cut_akhir_detail.stored_procedure', 'SP_LapUmurCrossCutAkhir');
        $syntax = (string) config('reports.umur_cross_cut_akhir_detail.call_syntax', 'exec');
