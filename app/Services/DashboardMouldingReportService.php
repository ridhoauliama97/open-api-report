...
class DashboardMouldingReportService
{
    ...
    public function buildReportData(string $startDate, string $endDate): array
    {
        ...
        if (
            ...
            throw new RuntimeException(
                'Kolom wajib tidak ditemukan di hasil SPWps_LapDashboardMoulding. '
                . 'Butuh kolom tanggal, jenis, grade, masuk, dan keluar. '
                . 'Kolom terdeteksi: ' . implode(', ', $detectedColumns),
            );
        }

        $orderedColumns = config('reports.dashboard_moulding.column_order', []);
        $orderedColumns = is_array($orderedColumns) ? array_values($orderedColumns) : [];
        $ctrDivisor = (float) config('reports.dashboard_moulding.ctr_divisor', 65);
        if ($ctrDivisor <= 0) {
            $ctrDivisor = 65.0;
        }
    ...
    private function resolveColumns(array $rows): array
    {
        ...
        return [
            ...
            'masuk' => $this->findMatchingKey($keys, ['mldmasuk', 'masukall', 'masuk', 'inflow', 'debit']),
            'keluar' => $this->findMatchingKey($keys, ['keluarall', 'mldkeluar', 'mldjual', 'jual', 'keluar', 'outflow', 'kredit']),
            's_akhir' => $this->findMatchingKey($keys, ['mldakhir', 'akhir', 's_akhir', 'stokakhir', 'saldoakhir']),
    ...
    {
        $connectionName = config('reports.dashboard_moulding.database_connection');
        $procedure = (string) config('reports.dashboard_moulding.stored_procedure');
        $syntax = (string) config('reports.dashboard_moulding.call_syntax', 'exec');
        $customQuery = config('reports.dashboard_moulding.query');
        $parameterCount = (int) config('reports.dashboard_moulding.parameter_count', 2);
        $parameterCount = max(0, min(2, $parameterCount));

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan dashboard moulding belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan dashboard moulding dikonfigurasi untuk SQL Server. '
                . 'Set DASHBOARD_MOULDING_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'DASHBOARD_MOULDING_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan DASHBOARD_MOULDING_REPORT_CALL_SYNTAX=query.',
                );
