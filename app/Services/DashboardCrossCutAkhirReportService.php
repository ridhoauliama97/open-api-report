...
class DashboardCrossCutAkhirReportService
{
    /**
    ...
    public function buildReportData(string $startDate, string $endDate): array
    {
        ...
        $orderedColumns = config('reports.dashboard_cross_cut_akhir.column_order', []);
        $orderedColumns = is_array($orderedColumns) ? array_values($orderedColumns) : [];
        $ctrDivisor = (float) config('reports.dashboard_cross_cut_akhir.ctr_divisor', 65);
        if ($ctrDivisor <= 0) {
            $ctrDivisor = 65.0;
        }
    ...
    private function resolveColumns(array $rows): array
    {
        ...
        return [
            ...
            'masuk' => $this->findMatchingKey($keys, ['ccmasuk', 'masukall', 'masuk', 'inflow', 'debit']),
            'keluar' => $this->findMatchingKey($keys, ['keluarall', 'cckeluar', 'ccjual', 'jual', 'keluar', 'outflow', 'kredit']),
            's_akhir' => $this->findMatchingKey($keys, ['ccakhir', 'akhir', 's_akhir', 'stokakhir', 'saldoakhir']),
    ...
    {
        $connectionName = config('reports.dashboard_cross_cut_akhir.database_connection');
        $procedure = (string) config('reports.dashboard_cross_cut_akhir.stored_procedure');
        $syntax = (string) config('reports.dashboard_cross_cut_akhir.call_syntax', 'exec');
        $customQuery = config('reports.dashboard_cross_cut_akhir.query');
