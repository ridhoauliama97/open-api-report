...
class DashboardSandingReportService
{
    ...
    public function buildReportData(string $startDate, string $endDate): array
    {
        ...
        if (
            ...
            throw new RuntimeException(
                'Kolom wajib tidak ditemukan di hasil SPWps_LapDashboardSanding. '
                . 'Butuh kolom tanggal, jenis, grade, masuk, dan keluar. '
                . 'Kolom terdeteksi: ' . implode(', ', $detectedColumns),
            );
        }

        $orderedColumns = config('reports.dashboard_sanding.column_order', []);
        $orderedColumns = is_array($orderedColumns) ? array_values($orderedColumns) : [];
        $ctrDivisor = (float) config('reports.dashboard_sanding.ctr_divisor', 65);
        if ($ctrDivisor <= 0) {
            $ctrDivisor = 65.0;
        }
    ...
    private function resolveColumns(array $rows): array
    {
        ...
        return [
            ...
            'masuk' => $this->findMatchingKey($keys, ['sandmasuk', 'masukall', 'masuk', 'inflow', 'debit']),
            'keluar' => $this->findMatchingKey($keys, ['keluarall', 'sandkeluar', 'sandjual', 'jual', 'keluar', 'outflow', 'kredit']),
            's_akhir' => $this->findMatchingKey($keys, ['sandakhir', 'akhir', 's_akhir', 'stokakhir', 'saldoakhir']),
    ...
    {
        $connectionName = config('reports.dashboard_sanding.database_connection');
        $procedure = (string) config('reports.dashboard_sanding.stored_procedure');
        $syntax = (string) config('reports.dashboard_sanding.call_syntax', 'exec');
        $customQuery = config('reports.dashboard_sanding.query');
        $parameterCount = (int) config('reports.dashboard_sanding.parameter_count', 2);
        $parameterCount = max(0, min(2, $parameterCount));

        if ($procedure === '' && !is_string($customQuery)) {
            throw new RuntimeException('Stored procedure laporan dashboard sanding belum dikonfigurasi.');
        }
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                'Laporan dashboard sanding dikonfigurasi untuk SQL Server. '
                . 'Set DASHBOARD_SANDING_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'DASHBOARD_SANDING_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan DASHBOARD_SANDING_REPORT_CALL_SYNTAX=query.',
                );
