...
class TimelineKayuBulatBulananReportService
{
    /**
    ...
    public function healthCheck(string $startDate, string $endDate): array
    {
        ...
        $expectedColumns = config('reports.timeline_kayu_bulat_bulanan.expected_columns', []);
    ...
    {
        $configKey = 'reports.timeline_kayu_bulat_bulanan';
        $connectionName = config("{$configKey}.database_connection");
        $procedure = (string) config("{$configKey}.stored_procedure", 'SP_LapTimelineKBBulanan');
        ...
        if ($driver !== 'sqlsrv' && $syntax !== 'query') {
            throw new RuntimeException(
                ...
                . 'Set TIMELINE_KAYU_BULAT_BULANAN_REPORT_CALL_SYNTAX=query jika ingin memakai query manual pada driver lain.',
            );
        }
        ...
        if ($syntax === 'query') {
            $query = is_string($customQuery) && trim($customQuery) !== ''
                ? $customQuery
                : throw new RuntimeException(
                    'TIMELINE_KAYU_BULAT_BULANAN_REPORT_QUERY belum diisi. '
                    . 'Isi query manual jika menggunakan TIMELINE_KAYU_BULAT_BULANAN_REPORT_CALL_SYNTAX=query.',
                );
        ...
        $sql = match ($syntax) {
            'exec' => $parameterCount === 0
                ...
                : ($parameterCount === 1
                    ? "SET NOCOUNT ON; EXEC {$procedure} @EndDate = ?"
