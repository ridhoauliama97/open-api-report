...
class MouldingHidupDetailReportService
{
    /**
     * @return array<int, array<string, mixed>>
     */
    public function fetch(?string $startDate = null, ?string $endDate = null): array
    {
        ...
        $normalized = array_map(function ($row): array {
            ...
            $noMoulding = trim((string) ($item['NoMoulding'] ?? $item['No_Moulding'] ?? $item['No'] ?? ''));
            ...
            return [
                'NoMoulding' => $noMoulding !== '' ? $noMoulding : null,
        ...
        usort($normalized, static function (array $a, array $b): int {
            ...
            return strcmp((string) ($a['NoMoulding'] ?? ''), (string) ($b['NoMoulding'] ?? ''));
        });
     ...
     */
    public function healthCheck(?string $startDate = null, ?string $endDate = null): array
    {
        $rows = $this->fetch($startDate, $endDate);
        $detectedColumns = array_keys($rows[0] ?? []);
        $expectedColumns = config('reports.moulding_hidup_detail.expected_columns', []);
    ...
    {
        $connectionName = config('reports.moulding_hidup_detail.database_connection');
        $procedure = (string) config('reports.moulding_hidup_detail.stored_procedure', 'SP_LapMouldingHidupDetail');
        $syntax = (string) config('reports.moulding_hidup_detail.call_syntax', 'exec');
        ...
        return $connection->select("SET NOCOUNT ON; {$sql}", array_slice($bindings, 0, $parameterCount));
    }
