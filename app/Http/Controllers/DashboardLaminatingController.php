...
use App\Http\Requests\ShowDashboardLaminatingRequest;
use App\Services\DashboardLaminatingReportService;
...
class DashboardLaminatingController extends Controller
{
    public function index(
        ShowDashboardLaminatingRequest $request,
        DashboardLaminatingReportService $reportService,
    ): View {
        ...
        return view('dashboard.laminating', [
            'startDate' => $startDate,
            'endDate' => $endDate,
    ...
    public function preview(
        ...
        return response()->json([
            'message' => 'Data dashboard laminating berhasil diambil.',
            'meta' => [
    ...
    public function health(
        ...
        return response()->json([
            'message' => $result['is_healthy']
                ? 'Struktur output SPWps_LapDashboardLaminating valid.'
                : 'Struktur output SPWps_LapDashboardLaminating berubah.',
            'meta' => [
                'start_date' => $startDate,
                'end_date' => $endDate,
                'TglAwal' => $startDate,
    ...
    public function download(
        ...
        $html = $pdfGenerator->renderHtml('dashboard.laminating-pdf', [
            'startDate' => $startDate,
            'endDate' => $endDate,
            'reportData' => $reportData,
        ...
        try {
            ...
            return response($pdf, 200, ['Content-Type' => 'application/pdf', 'Content-Disposition' => 'attachment; filename="Dashboard Laminating"']);
    ...
    private function resolveDates(ShowDashboardLaminatingRequest $request): array
    {
        $defaultEndDate = now()->endOfMonth()->toDateString();
        $defaultStartDate = now()->startOfMonth()->toDateString();
